...
<div class="col-md-10 col-md-offset-1">
    <form id="generate_scheme_form">
        <div class="thumbnail" style="font-size: 14px;">
            ...
            <span class="col-md-4">Scheme Code: <?php echo $scheme->scheme_code ?></span>
            ...
            <span class="col-md-4"><div class="pull-right">Settlement Type: <?php echo "Payout"; ?></div></span><div class="clearfix"></div><hr>
            <span class="col-md-12"><h5><?php echo str_replace('_', ' ', $schemetype->scheme_type) ?> Scheme. Details as mentioned below,</h5></span>
            <center>The Price drop is <b><?php echo empty($scheme->is_gst_include) ? "Excluding GST" : "Including GST"; ?></b></center>
            <input type="hidden" name="is_gst_include" value="<?php echo $scheme->is_gst_include ?>" />
            <table class="table table-bordered table-condensed" style="margin: 0">
                <thead>
                    <th>Model</th>
                    <th>Last Purchase Price</th>
                    <th style="background-color: #cdfbee">Price Increase</th>
                    <th>Excluding gst amt</th>
                    <th>IGST%</th>
                    <th style="background-color: #cdfbee">Effective Price change</th>
                    <th>New Price</th>
                </thead>
                <tbody>
...
<div class="clearfix"></div>
<button class="btn btn-primary btn-sm pull-right" id="btn_export_report" onclick="javascript:xport.toCSV('Price_Increase_Scheme');" style="margin: 0; display: none"><span class="fa fa-file-excel-o"></span> Export</button><div class="clearfix"></div><br>
...
$(document).ready(function(){
    $(document).on("click", "#generate_scheme", function (event) {
        ...
        if(confirm('Do you want to generate cailm data')){
            ...
            $.ajax({
                url: "<?php echo base_url('Scheme/generate_price_increase_claim') ?>",
                method: "POST",
                data: serialized,
                dataType: 'json',
...
//                        $('#footer_action').html('<?php echo str_replace("_", " ", $schemetype->scheme_type) ?> Claim generated successfully <center><a class="btn btn-primary" id="view_claimed_report">View Report</a></center>');
                        var leb = '<center>Price Increase claim generated. Last generated on <?php echo date('d-m-Y h:i a') ?></center>\n\
                                    <a class="btn btn-sm btn-primary" id="view_claimed_report"><i class="mdi mdi-note"></i> View Report</a>\n\
    ...
    $(document).on("click", "#view_claimed_report", function (event) {
        ...
        var scheme_type = 'Price_Increase_Scheme';
        $.ajax({
