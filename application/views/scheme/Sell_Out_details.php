...
<div class="col-md-10 col-md-offset-1">
    <form id="generate_scheme_form">
        <div class="thumbnail" style="font-size: 14px;">
            ...
            <span class="col-md-3">Claim Target: <b> <?php echo empty($scheme->claim_target) ? "Qty" : "Value"; ?></b></span>
...
//                    echo '<pre>'.print_r($settlement_data,1).'</pre>';
                    ...
                    <tr>
                        ...
                        <?php if($scheme->claim_target == 1){ ?>
                            <td><?php if($scheme->has_slabs == 0){ echo $sd['ach_'.$id][0]->mop;}else{ echo $sd['total_ach_'.$scheme->id_scheme][0]->mop; }  ?></td>
                        <?php  }else{ ?>
                            ...
                            <?php }elseif($scheme->settlement_type == 1){
                                    ...
                                    <td class="p-2"><div class="col-md-6 thumbnail btn-sm" style="background-color: #c3ffae; color: #000; margin-bottom: 0">
                                        <?php  
                                         if($scheme->has_slabs == 0){ 
                                             ...
                                             if($sale_count>0){                                                                                     
                                                if($sd['min']==$sd['max'] && $sale_count >= $sd['max']){
                                    ...
                                    <?php }elseif($scheme->claim_target == 1){ ?>
                                        ...
                                        <?php  
                                         if($scheme->has_slabs == 0){ 
                                             ...
                                             $payout=0;
                                             $sale_count= $sd['total_ach_'.$scheme->id_scheme][0]->mop;
                                             if($sale_count>0){                                                                                     
                                                if($sd['min']==$sd['max'] && $sale_count >= $sd['max']){
                                   ...
                                   <?php  } 
                                    ...
                                    <?php }else{ ?>
                                        ...
                                        if($sd['min']==$sd['max'] && $sale_qty >= $sd['max']){
                                            $payout_per= $sd['payout_per'];
            ...
            <div id="footer_action" class="text-success">
...
<button class="btn btn-primary btn-sm pull-right" id="btn_export_report" onclick="javascript:xport.toCSV('Sell_Out_Scheme');" style="margin: 0; display: none"><span class="fa fa-file-excel-o"></span> Export</button><div class="clearfix"></div><br>
...
$(document).ready(function(){
    $(document).on("click", "#generate_scheme", function (event) {
        ...
        if(confirm('Do you want to generate cailm data')){
            ...
            $.ajax({
                url: "<?php echo base_url('Scheme/generate_sell_out_claim') ?>",
                method: "POST",
                data: serialized,
                dataType: 'json',
...
//                        $('#footer_action').html('<?php // echo str_replace("_", " ", $schemetype->scheme_type) ?> Claim generated successfully <center><a class="btn btn-primary" id="view_claimed_report">View Report</a></center>');
                        var leb = '<center>Sell Out claim generated. Last generated on <?php echo date('d-m-Y h:i a') ?></center>\n\
                                    <a class="btn btn-sm btn-primary" id="view_claimed_report"><i class="mdi mdi-note"></i> View Report</a>\n\
    ...
    $(document).on("click", "#view_claimed_report", function (event) {
        ...
        var scheme_type = 'Sell_Out_Scheme';
        $.ajax({
            url: "<?php echo base_url('Scheme/view_sell_out_claim') ?>",
            method: "POST",
