...
<center><h3 style="margin-top: 0"><span class="mdi mdi-sitemap fa-lg"></span> Shopact and Gst Certificate </h3></center><div class="clearfix"></div><hr>
		...
		<form id="branch_shopact_gst_form" name="branch_rent_form" onsubmit="return CommonFunction(branch_rent_form)">
			<div class="col-md-10 thumbnail col-md-offset-1">
				...
				<div class="col-md-2">
					...
					<label class="col-md-2">Shop Act Document</label>
					<div class="col-md-3">
						<input type="file" class="form-control" placeholder="Rent Document" name="shopact_doc" id="shopact_doc" />
					</div>
					<?php if($this->session->userdata('role_name')=='Legal'){ 
						if(!empty($rentow_details['shopact_doc'])){
							?>
							<label class="col-md-1">
								<a href="<?php echo base_url().$rentow_details['shopact_doc']; ?>" target="_blank"><button  type="button">View</button></a>
							</label>
						<?php }
					} ?>
					<label class="col-md-2">Gst Certificate</label>
					<div class="col-md-3">
						<input type="file" class="form-control" placeholder="Rent Document" name="gstcert_doc" id="gstcert_doc" />
					</div>
					<?php if($this->session->userdata('role_name')=='Legal'){ 
						if(!empty($rentow_details['gstcert_doc'])){
							?>
							<label class="col-md-1"><a href="<?php echo base_url().$rentow_details['gstcert_doc']; ?>" target="_blank"><button type="button">View</button></a></label>
						<?php } 
					} ?>
			...
			$(document).ready(function () {
				$("#branch_shopact_gst_form").validate({
					...
					submitHandler: function(form) {
						...
						$.ajax({
							url:base_url + 'branch-shopact-gst-store',
							type: "POST",
							data: formData,
							async: true,
							dataType:"JSON",
							...
							success:function(response)
							{
								if(response.status)
								{
									...
									setTimeout(function(){
										window.location.href = base_url + 'branch-shopact-gst';
									},2000);
								}else{
									alert(response.message);
								}
