    ...
    <script>
        ...
        $(document).on('keyup', 'input[id=barcode]', function(e) {
            ...
            if (keyCode === 13 && val !== '') {
                ...
                $.ajax({
                    url:"<?php echo base_url() ?>Stock/ajax_check_valid_barcode_for_transfer",
                    method:"POST",
...
$(ce).closest('td').parent('tr').find(".scanned1").append('<input type="text" class="btn btn-sm btn-warning scanned_imei" id="scanned_imei" readonly="" style="margin:2px; width: 160px;background-color: #0e10aa;opacity: 0.8;" value="'+val+'" href="javascript:void(0);"></input>');
    ...
    return false;
        ...
        $(document).on("click", ".submit-transfer", function(event) {   
            ...
            if($(".idtvendors").val()==9 && $('#vehicle_no').val()==''){
             alert("Shipment is aborted. If transport is self then enter vehicle no and po lr no");            
             return false;
         }
            ...
            $.ajax({
                ...
                {                            
                    if(data.data === 'success'){
                                ...
                                $.ajax({
                                    url: "<?php echo base_url() ?>Generate-eway-bill/"+tobranch+"/"+frombranch+"/"+idtransfer+"/transfer",
                                    method: "POST",
                                    data: serialized,
                                    dataType:'json',
                                    success: function (data)
                    ...
                    <tbody id="product_data" style="border: 1px solid #C8D4D4">
                        ...
                        if($role_type!=0){
                            ...
                            <div class="thumbnail" >            
                                ...
                                <div class="col-md-2"><input type="text" class="form-control input-sm no_of_boxes" id="no_of_boxes" name="no_of_boxes" placeholder="No of Boxes" required=""/></div><br>
