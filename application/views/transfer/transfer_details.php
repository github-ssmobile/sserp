                ...
                <tbody>
                    ...
                    foreach ($transfer_product as $product) { ?>
                        <tr>
                            ...
                            <td><?php echo round(($product->qty)*($product->landing)) ?></td>
                        </tr>
                        <?php $i++; } ?>
                    </tbody>
                <?php }else { ?>           
...
<?php } 
$ewayinv_data= $this->common_model->getSingleRow('eway_einvoice_data',array('idoutword_no'=>$transfer->id_transfer));
?>
<div style="position: fixed; left: 250px; bottom: 70px;"><a  href="<?php echo base_url().'Print-e-way/'.$ewayinv_data['ewb_no'].'/'.$transfer->transfer_from;?>" class="btn btn-success btn-md" data-toggle="tooltip" data-placement="top" title="Print E-Way Bill" target="_blank"><i class="fa fa-print " style="color: blue"></i>&nbsp;&nbsp;Print EWAY</a></div>
<?php if($ewayinv_data['bill_type']=='1'){ ?>                                 
 <div style="position: fixed; left: 400px; bottom: 70px;"> <a href="<?php echo base_url().'Print-e-invoice/'.$ewayinv_data['idoutword_no'].'/'.$transfer->transfer_from;?>" target="_blank">
    <button type="button" class="btn btn-md btn-success textalign gen-eway-bill"><i class="fa fa-print " style="color: blue"></i>&nbsp;&nbsp;E-Invoice</button>
</a></div>
<?php } ?>
