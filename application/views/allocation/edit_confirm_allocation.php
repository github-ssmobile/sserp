...
$(document).on("click", ".remove_allocation", function(event) {
    ...
    $('.stockallocation').click(function(){
        ...
        $.ajax({
            ...
            method:"POST",
            data:{idstock : idstock, date:date, bname: bname, entry_time: entry_time,idbranch:idbranch,allocation_type:allocation_type,isconfirm:1},
            success:function(data)
            {
    ...
    $(document).on("click", ".allocationform", function (event) {
        ...
        $(document).on("change", "input[type='checkbox']", function (event) {         
            ...
            if($(this).prop("checked") == true){
               ...
               var id=$(this).val();
               aloc_ids = jQuery.grep(aloc_ids, function(value) { return value !== id; });
            }
        ...
        $(document).on("click", ".confirm_a", function (event) {
                ...
                if (confirm('Do you want to Confirm this Allocations!!')) {                    
                    ...
                    url: "<?php echo base_url() ?>Stock_allocation/confirm_allocation",
                    method: "POST",                
                    data: {allocation_id:aloc_ids ,status:status  },        
                    dataType:'json',
                    success: function (data)
                    {
                       if(data.data === 'success'){
