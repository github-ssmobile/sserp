...
//            if(datefrom == '' || dateto == '' || idbranch == ''){
            if(datefrom == '' || dateto == ''){
                ...
                $.ajax({
                    url:"<?php echo base_url() ?>Report/ajax_new_daybook_summary_report",
                    method:"POST",
                    data:{datefrom : datefrom, dateto : dateto, idbranch: idbranch,branches: branches},
                    success:function(data)
                    {
                        $(".daybook_sale").html(data);
                    }
                });
            }
        });
...
<div class="col-md-6 col-sm-10 col-xs-10"><center><h3 style="margin: 0"><span class="mdi mdi-clipboard-text fa-lg"></span> Daybook Summary Report</h3></center></div>
<div class="col-md-2 col-sm-2 col-xs-2"><button class="btn btn-primary btn-sm pull-right" onclick="javascript:xport.toCSV('daybook_sale<?php echo date('d-m-Y h:i a') ?>');" style="margin: 0"><span class="fa fa-file-excel-o"></span> Export</button></div><div class="clearfix"></div><br>
...
<div class="thumbnail" style="height: 550px; font-size: 13px; overflow: auto; margin-top: 5px; padding: 0">
    <table id="daybook_sale<?php echo date('d-m-Y h:i a') ?>" class="table table-condensed table-bordered table-hover daybook_sale" style="margin: 0"></table>
</div>
