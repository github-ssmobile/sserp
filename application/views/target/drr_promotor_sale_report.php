    ...
    $(document).ready(function (){
        $('#btnreport').click(function (){
            ...
            if(idbranch == '' && idzone == ''){
                ...
                if(from != '' && idpcat !='' && idbranch != ''){
                   $.ajax({
                        url:"<?php echo base_url() ?>Target/ajax_get_drr_promotor_sale_report_byidbranch",
                        method:"POST",
                        data:{from: from, idpcat: idpcat, idbranch: idbranch, allbranches: allbranches, allpcats: allpcats},
                ...
                if(from != '' && idpcat !='' && idzone != ''){
                   $.ajax({
                        url:"<?php echo base_url() ?>Target/ajax_get_drr_promotor_sale_report_byidzone",
                        method:"POST",
                        data:{from: from, idpcat: idpcat, idzone: idzone, allzone: allzone, allpcats: allpcats},
                        success:function(data)
                        {
                            $('#target_data').html(data);
                        }
...
<center><h3><span class="mdi mdi-checkbox-marked-outline fa-lg"></span> DRR Promotor Sale Report </h3></center><hr>
    ...
    <div  style="padding: 20px 10px; margin: 0">
       ...
       <div class="clearfix"></div><hr>
        ...
        <div class="col-md-2 col-sm-2 col-xs-2 pull-right ">
            <button class="btn btn-info btn-sm pull-right " onclick="javascript:xport.toCSV('DRR_Promotor_Sale_Report');" style="margin: 0"><span class="fa fa-file-excel-o"></span> Export To Excel</button>
        </div> 
