...
<script>
    $(document).ready(function (){
        $('#btnreport').click(function (){
            ...
            if(idbranch == '' && idzone == ''){
                ...
                if(idpcat !='' && idbranch != ''){
                   $.ajax({
                        url:"<?php echo base_url() ?>Target/ajax_get_lmtd_brand_sale_report_byidbranch",
                        method:"POST",
                ...
                if(idpcat !='' && idzone != ''){
                   $.ajax({
                        url:"<?php echo base_url() ?>Target/ajax_get_lmtd_brand_sale_report_byidzone",
                        method:"POST",
    ...
    function sortTable(){
	var rows = $('#LMTD_branch_sale_report tbody  tr').get();
	...
	$.each(rows, function(index, row) {
	    $('#LMTD_branch_sale_report').children('tbody').append(row);
        });
    }
    function sortTablezone(){
	var rows = $('#LMTD_brand_sale_report tbody  tr').get();
	rows.sort(function(a, b) {
	...
	$.each(rows, function(index, row) {
	    $('#LMTD_brand_sale_report').children('tbody').append(row);
        });
    }
...
<center><h3><span class="mdi mdi-checkbox-marked-outline fa-lg"></span> LMTD Brand Sale Report </h3></center><hr>
    ...
    <div  style="padding: 20px 10px; margin: 0">
       ...
       <div class="clearfix"></div><hr>
        ...
        <div class="col-md-2 col-sm-2 col-xs-2 pull-right ">
            <button class="btn btn-info btn-sm pull-right " onclick="javascript:xport.toCSV('LMTD_brand_sale_report');" style="margin: 0"><span class="fa fa-file-excel-o"></span> Export To Excel</button>
        </div> 
