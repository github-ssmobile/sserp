    ...
    $(document).ready(function (){
        $('#btnreport').click(function (){
            ...
            if(idbranch == '' && idzone == ''){
                ...
                if(monthyear != '' && idpcat !='' && idbranch != ''){
                   $.ajax({
                        url:"<?php echo base_url() ?>Target/ajax_get_promotor_target_vs_ach_byidbranch",
                        method:"POST",
                        data:{monthyear: monthyear, idpcat: idpcat, idbranch: idbranch, allbranches: allbranches, allpcats: allpcats},
                ...
                if(monthyear != '' && idpcat !='' && idzone != ''){
                   $.ajax({
                        url:"<?php echo base_url() ?>Target/ajax_get_promotor_target_vs_ach_byidzone",
                        method:"POST",
                        data:{monthyear: monthyear,idpcat: idpcat, idzone: idzone, allzone: allzone, allpcats: allpcats},
                        success:function(data)
                        {
                            $('#target_data').html(data);
                        }
...
<center><h3><span class="mdi mdi-checkbox-marked-outline fa-lg"></span> Promoter Target vs Achievement Report </h3></center><hr>
    ...
    <div  style="padding: 20px 10px; margin: 0">
        ...
        <div class="col-md-2">
            ...
            <select name="idpcat" class="form-control chosen-select" id="idpcat">
                ...
                <?php foreach ($product_cat_data as $pcat){ if($pcat->id_product_category == 1){  ?>
       ...
       <div class="clearfix"></div><hr>
        ...
        <div class="col-md-2 col-sm-2 col-xs-2 pull-right ">
            <button class="btn btn-info btn-sm pull-right " onclick="javascript:xport.toCSV('Promoter_Target_vs_Achievement_Report');" style="margin: 0"><span class="fa fa-file-excel-o"></span> Export To Excel</button>
        </div> 
        <div class="clearfix"></div><br>
        <div id="target_data" style="overflow-x: auto;height: 600px;">
        </div>
    </div>
</div>
