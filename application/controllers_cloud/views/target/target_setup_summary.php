    ...
    $(document).ready(function (){
        $('#btnreport').click(function (){
            ...
            if(idbranch == '' && idzone == ''){
                ...
                if(slabmonth !='' && idpcat !='' && idbranch != ''){
                   $.ajax({
                        url:"<?php echo base_url() ?>Target/ajax_get_target_setup_summary_byidbranch",
                        method:"POST",
                        data:{idpcat: idpcat, idbranch: idbranch, allbranches: allbranches, allpcats: allpcats, slabmonth: slabmonth},
                ...
                if(slabmonth !='' && idpcat !='' && idzone != ''){
                   $.ajax({
                        url:"<?php echo base_url() ?>Target/ajax_get_target_setup_summary_byidzone",
                        method:"POST",
                        data:{idpcat: idpcat, idzone: idzone, allzone: allzone, allpcats: allpcats, slabmonth: slabmonth},
                        success:function(data)
                        {
                            $('#target_data').html(data);
                        }
...
<center><h3><span class="mdi mdi-checkbox-marked-outline fa-lg"></span> Target Setup Summary </h3></center><hr>
    ...
    <div  style="padding: 20px 10px; margin: 0">
       ...
       <div class="clearfix"></div><hr>
        ...
        <div class="col-md-2 col-sm-2 col-xs-2 pull-right ">
            <button class="btn btn-info btn-sm pull-right " onclick="javascript:xport.toCSV('target_setup_summary_report');" style="margin: 0"><span class="fa fa-file-excel-o"></span> Export To Excel</button>
        </div> 
