...
//        die(print_r($_POST));
        ...
        $idbrand = $this->input->post('idbrand');
        $sale_data = $this->Sale_model->ajax_get_sale_data_byfilter($from, $to, $idbranch, $idpcat, $idbrand);
...
//        die('<pre>'.print_r($sale_return_data,1).'</pre>');
        ...
        <table id="sale_report" class="table table-bordered table-striped table-condensed table-info">
            ...
            <tbody class="data_1">
                ...
                <tr>
                    <td colspan="25" style="background-color: #9999ff;color: #FFFFFF"> <b> Sales Return </b> </td>
                                ...
                                <tbody>
                                    ...
                                    <tr class="product_row">
                                        <td>
                                            ...
                                            <input type="text" id="activation_code" class="activation_code form-control input-sm" name="activation_code[]" value="<?php echo $product->activation_code ?>" placeholder="Enter Activation Code" />
                                ...
                                <tbody>
                                    <?php // die('<pre>'.print_r($sale_payment,1).'</pre>');
                                     $sum = 0; $cashr=0; foreach ($sale_payment as $payment){
                                        if((($payment->credit_type == 1 && $payment->payment_receive == 1) || ($sale->date != date('Y-m-d') && $payment->idpayment_mode == 1)) && ($this->session->userdata('id_users') != 32)) {
//                                        if(($payment->credit_type == 1 && $payment->payment_receive == 1) || ($sale->date != date('Y-m-d') && $payment->idpayment_mode == 1) || ($this->session->userdata('id_users') != 32)) {
//                                        if(0) { ?>
...
//                                            if($cashr != $mode->id_paymentmode){ ?>
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="hidden" name="idbranch" value="<?php echo $sale->idbranch ?>">
        ...
        if(array_sum($edited_idsaleproduct) > 0){
            for($i=0;$i<count($edited_idsaleproduct);$i++){
                if($edited_idsaleproduct[$i] != 0){
                    ...
                    if($qty != $old_qty || $price != $old_price || $discount_amt != $old_discount_amt || $insurance_imei_no != $old_insurance_imei_no || $activation_code != $old_activation_code){
