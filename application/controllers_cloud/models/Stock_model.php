...
//        die($this->db->last_query());
    ...
    public function get_imei_stock_by_GPBB($idgodown,$idbrand,$idproductcategory,$idbranch,$iswarehouse,$idbranchs,$type=1) {              
        $this->db->select('st.idbranch,st.temp_idbranch,st.imei_no,pc.product_category_name,brd.brand_name,mv.id_variant,mv.full_name, `b`.`branch_name`, st.qty,g.godown_name,c.category_name');
...
//        die(print_r($this->db->last_query()));
    ...
    public function get_store_stock_analysis($idgodown,$idbranch,$idbranches,$days,$idbrand,$idproductcategory,$type) {
        $to=date('Y-m-d');
        $day='-'.$days.' days';                 
        $from=date('Y-m-d', strtotime("$day", strtotime($to)));           
        $this->db->from('model_variants mv');    
...
//        die($this->db->last_query());
   ...
   public function get_sale_stock_analysis($idgodown,$days,$idbrand,$idproductcategory,$idcategory) {        
...
//        die($this->db->last_query());
    ...
    public function get_ageing_imei_stock_by_GPBB($idgodown,$idbrand,$idproductcategory,$idbranch,$iswarehouse,$idbranchs,$type=1) {              
        $this->db->select('st.idbranch,st.temp_idbranch,st.imei_no,pc.product_category_name,brd.brand_name,mv.id_variant,mv.full_name, `b`.`branch_name`, st.qty,g.godown_name');
        $this->db->from('stock st'); 
        if($type==1){
            $this->db->join('branch b', 'st.idbranch=b.id_branch','left');    /// for current stock
        }else{        
...
//        die($this->db->last_query());
    ...
    public function get_focus_model_imei_stock_by_GPBB($idgodown,$idbrand,$idproductcategory,$idbranch,$iswarehouse,$idbranchs,$type=1) {              
        $this->db->select('st.idbranch,st.temp_idbranch,st.imei_no,pc.product_category_name,brd.brand_name,mv.id_variant,mv.full_name, `b`.`branch_name`, st.qty,g.godown_name,focus_model_stock.incentive_amount');
        $this->db->from('stock st'); 
        if($type==1){
            $this->db->join('branch b', 'st.idbranch=b.id_branch','left');    /// for current stock
        }else{        
    ...
    public function get_brand_wise_placement_norms($days,$product_category,$branch){
        ...
        $this->db->select('b.id_brand, b.brand_name, stk.curr_stock,sp.sale_qty,ub.promotor,intra_stk.intra_stock');
        $this->db->where('b.norm_sequence !=', 0);
        $this->db->where('b.id_brand !=', 76);
        $this->db->join("(select sum(st.qty) as curr_stock, st.idbrand from stock st, category c WHERE st.idproductcategory = $product_category and st.idbranch = $branch and c.norm_sequence = 0 and st.idcategory = c.id_category group by st.idbrand) stk", 'stk.idbrand = b.id_brand', 'left');
        $this->db->join("(select sum(st.qty) as intra_stock, st.idbrand from stock st, category c WHERE st.idproductcategory = $product_category and st.temp_idbranch = $branch and c.norm_sequence = 0 and st.idcategory = c.id_category group by st.idbrand) intra_stk", 'intra_stk.idbrand = b.id_brand', 'left');
        $this->db->join("(select sum(s.qty) as sale_qty,s.idbrand from sale_product s, category c WHERE s.idproductcategory = $product_category and s.idbranch = $branch and c.norm_sequence = 0 and  s.idcategory = c.id_category and s.date between '$from' and '$to' GROUP BY s.idbrand) sp", 'sp.idbrand=b.id_brand', 'left');                
        $this->db->join("(select count(p.id_user_has_brand) as promotor,p.idbrand,u.iduserrole from user_has_brand p, users u WHERE p.iduser = u.id_users and u.iduserrole = 17 and u.idbranch = $branch group by p.idbrand) ub", 'ub.idbrand=b.id_brand', 'left');                
...
//        $this->db->order_by('b.norm_sequence');
    ...
    public function get_category_wise_placement_norms($days,$product_category,$branch){
        ...
        $this->db->where('b.norm_sequence !=', 0);
        $this->db->join("(select sum(st.qty) as curr_stock, st.idcategory from stock st WHERE st.idproductcategory = $product_category and st.idbranch = $branch group by st.idcategory) stk", 'stk.idcategory = b.id_category', 'left');
        $this->db->join("(select sum(st.qty) as intra_stock, st.idcategory from stock st WHERE st.idproductcategory = $product_category and st.temp_idbranch = $branch group by st.idcategory) intra_stk", 'intra_stk.idcategory = b.id_category', 'left');
        $this->db->join("(select sum(s.qty) as sale_qty,s.idcategory from sale_product s WHERE s.idproductcategory = $product_category and s.idbranch = $branch and s.date between '$from' and '$to' GROUP BY s.idcategory) sp", 'sp.idcategory=b.id_category', 'left');                
...
//        die(print_r($strbr));
        if($idzone == 'all'){ //Overall Zone
            ...
            $this->db->join("(select count(p.id_user_has_brand) as promotor,p.idbrand,u.iduserrole from user_has_brand p, users u WHERE u.idbranch in($strbr) and p.iduser = u.id_users and u.iduserrole = 17   group by p.idbrand) ub", 'ub.idbrand=b.id_brand', 'left');                
        ...
        elseif ($idzone == 'allzone') {
            ...
            $this->db->join("(select count(p.id_user_has_brand) as promotor,p.idbrand,u.iduserrole,u.idbranch,brr.idzone from user_has_brand p, users u,branch brr WHERE u.idbranch = brr.id_branch and brr.idzone in($strbr) and  p.iduser = u.id_users and u.iduserrole = 17   group by p.idbrand,brr.idzone) ub", 'ub.idbrand=b.id_brand and ub.idzone=z.id_zone', 'left');                
    ...
    //        $this->db->select('bnd.id_branch,bnd.branch_name,b.id_brand,zone.zone_name, b.brand_name,"0" as id_category ,stk.curr_stock,sp.sale_qty,ub.promotor,sn.quantity');
            ...
            $this->db->join("(select count(p.id_user_has_brand) as promotor,p.idbrand,u.iduserrole,u.idbranch from user_has_brand p, users u WHERE p.iduser = u.id_users and u.iduserrole = 17   group by p.idbrand,u.idbranch) ub", 'ub.idbrand=b.id_brand and ub.idbranch=bnd.id_branch', 'left');                
...
//         die(print_r($_POST));
        ...
        if($idzone == 'all'){ 
            ...
            $this->db->join("(select s.quantity,s.idcategory from stock_norms s WHERE s.idbranch in($strbr) and s.idproductcategory = $product_category GROUP BY s.idcategory) sn", 'sn.idcategory = b.id_category', 'left');  
            $this->db->order_by('b.norm_sequence');
            $this->db->from('category b');
            $query = $this->db->get();  
            return $query->result();
...
//            die("dfg");
            ...
            $this->db->join("(select count(p.id_user_has_brand) as promotor,p.idbrand,u.iduserrole,u.idbranch from user_has_brand p, users u, brand b WHERE b.norm_sequence =0 and p.iduser = u.id_users and u.iduserrole = 17 and p.idbrand = b.id_brand GROUP BY u.idbranch) ub", 'ub.idbranch = br.id_branch', 'left');                
            $this->db->where('br.idzone = zone.id_zone')->from('zone');
...
//            die("hii");
            ...
            $this->db->join("(select count(p.id_user_has_brand) as promotor,p.idbrand,u.iduserrole,u.idbranch,brr.idzone from user_has_brand p, users u, brand b, branch brr WHERE u.idbranch = brr.id_branch and brr.idzone in($strbr) and b.norm_sequence =0 and p.iduser = u.id_users and u.iduserrole = 17 and p.idbrand = b.id_brand GROUP BY brr.idzone) ub", 'ub.idzone = z.id_zone', 'left');                
