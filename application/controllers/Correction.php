...
//        $q['customer_data'] = $this->Correction_model->get_customer_data();
        ...
        if($this->session->userdata('level') == 2){
            $branch = $_SESSION['idbranch'];
        }
        elseif($this->session->userdata('level') == 3){
            $branch_data = $this->General_model->get_branches_by_user($_SESSION['id_users']);
            foreach ($branch_data as $bdata){
                $branch[] = $bdata->id_branch;
            }
        }else{
        ...
        $q['correction_request_data'] = $this->Correction_model->get_pending_correction_request($branch);
...
//        die('<pre>'.print_r($_POST,1).'</pre>');
    ...
    public function branch_pending_correction(){
        ...
        $q['correction_request_data'] = $this->Correction_model->get_branch_pending_correction_request($branch);
            ...
            <table class="table table-bordered table-condensed" id="helpline_report">
                ...
                <tbody class="data_1">
                    ...
                    <tr>
                        ...
                        <td><?php echo $cdata->updted ?></td>
                        <td><?php echo date('Y-m-d', strtotime($cdata->updated_entrytime)); ?></td>
    ...
    public function invoice_delete() {
        ...
        if(count($sale_data)>0){
            $dup=$this->Correction_model->get_duplicate_invoice();            
            $flag=1;
//            foreach ($dup as $in){
//                if($invoice_no==$in->inv_no){
//                    $flag=1;
//                    break;
//                }
//            }            
            if($flag){                
                if($this->Correction_model->delete_duplicate_invoice($sale_data[0]->id_sale)){
                    echo 'DONE';
                }else{
                    echo 'FAIL';
                }
