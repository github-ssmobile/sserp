...
//        echo print_r($_POST);
        if(count($models)){
            ...
            foreach($models as $model){
                if($model->idgodown == 1 || $model->idgodown == 6){
...
//        $idgodown = 4;
        ...
        $models = $this->Sale_model->ajax_stock_data_byimei_branch($imei_no, $idbranch); 
        $service=$this->Service_model->get_service_details_byimei($imei_no);
        $flag=1;
        if(count($service)>0){
            foreach ($service as $sr){
                if($sr->process_status==11 || $sr->process_status==17 || $sr->process_status==19){                     
                }else{ $flag=0; break; }
            }
        }
        if(($models[0]->idgodown==1 || $models[0]->idgodown==6) && ($flag==1)){            
    ...
    }else{ $this->session->set_flashdata('reject_data', 'IMEI not in NEW/Online godown');        
            return redirect('Service/service_inward');
        }
    }
...
//      die('<pre>'.print_r($_POST,1).'</pre>');
       ...
       $models = $this->Sale_model->ajax_stock_data_byimei_branch($imei_no, $idbranch);      
        ...
        if(count($models)>0){            
            $this->session->set_flashdata('reject_data', 'IMEI already inwarded into Stock');        
            return redirect('Service/service_inward');
        }else{        
        if($idcustomer==0){
            ...
            <table class="table table-condensed table-striped table-bordered table-hover" style="margin-bottom: 0;">
                ...
                <tbody class="data_1">
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if($this->session->userdata('idrole') == 39 && $stock->process_status==13){ ?>
...
//          'iddoainward' => $iddoainward
        ...
        $updata_service = array(
            'process_status' => 17, // Processed by Executive
            'closed' => $date,
...
//        die('<pre>'.print_r($q['service_stock'],1).'<pre>');
...
//        die('<pre>'.print_r($_POST,1).'<pre>');
    ...
    public function reject_servcice_at_ho_from_branch() {
//        die('<pre>'.print_r($_POST,1).'<pre>');
        // Received at HO from Branch = 5
        $this->db->trans_begin();
        $imei_no = $this->input->post('imei_no');
        ...
        $service_trasf = array(
            'shipment_reject_remark' => $this->input->post('shipment_received_remark'),
            'shipment_reject_date' => $date,
            'shipment_reject_by' => $shipment_received_by,
            'shipment_received_entry_time' => $datetime,
            'status' => 2,
        );
        $this->Service_model->update_service_transfer($idservice_transfer, $service_trasf);
        ...
        for($i=0; $i < $count; $i++){
            // update stock
            $service_row = $this->Service_model->get_service_row_byid($idservice[$i]);
            $update_stock = array(
                'idbranch' => $service_row->idbranch,
                'temp_idbranch' => 0,
                'transfer_from' => $idbranch
            );
            $this->Service_model->update_stock_byimei($imei_no[$i],$update_stock);
            ...
            $imei_history[] = array(
                'imei_no' => $imei_no[$i],
                'entry_type' => 'Service - Rejected by HO',
                'entry_time' => $datetime,
                'date' => $date,
                'idbranch' => $service_row->idbranch,
                'idgodown' => 4,
...
//        die('<pre>'.print_r($_POST,1).'<pre>');
    ...
    public function ajax_sent_to_local(){        
        ...
        $idservice_executive=NULL;
        if($branch_data->idservice_executive!=NULL || $branch_data->idservice_executive > 0){
            $idservice_executive=$branch_data->idservice_executive;
        }
        ...
        $data = array(
            'process_status' => 2,
            'branch_to_local' => $date,
            'carename' => $care,
            'idservice_excecutive' => $idservice_executive,
            'allocate_to_excecutive' => $date
        );
        $imei_history[]=array(
            'imei_no' => $imei_no,
            'entry_type' => 'Send to Local care - '.$care,
            'entry_time' => $datetime,
            'date' => $date,
            'idbranch' => $idbranch,
            'idgodown' => 4,
            'idvariant' => $idvariant,
            'idimei_details_link' => 16,
            'idlink' => $idservice,
            'iduser' => $id_users,
        );
...
//            die(print_r($q));
    ...
    public function product_replacement_form() {
        ...
        <div class="col-md-10 col-md-offset-1 col-lg-10 col-lg-offset-1" style="padding: 0">
            ...
            <?php }elseif($doa_return_type==4){
                 $title="Product Inward/Upgrade Form";?>    
                <center><span style="color: #9b0c13"><i class="fa fa-user"></i> Credit note received at Branch from Care </span></center><br><br>
            ...
            <div class="neucard shadow-inset border-light p-4">
                <div class="shadow-soft border-light rounded p-4" style="background-color: #fff">
                    ...
                    <?php if($doa_return_type==1){ ?>
                        ...
                        <div class="clearfix"></div><br>
                        <div class="col-md-2">DOA Letter </div>
                        <div class="col-md-4"><input type="file" name="doa_letter_file" id="doa_letter_file" class="form-control input-sm doa_letter_file" required="" /></div>
                        <?php }elseif($doa_return_type==4){ ?>
                        <div class="col-md-2 col-sm-4">Credit Note No</div>
                        <div class="col-md-4 col-sm-4">
                            <input type="text" class="form-control input-sm cn_no" name="cn_no" id="cn_no" placeholder="Enter CN No" required="" />
                        </div>
                        <div class="col-md-2 col-sm-4">Date</div> 
                        <div class="col-md-4 col-sm-4">
                            <input type="text" class="form-control input-sm cn_date" data-provide="datepicker" name="cn_date" id="cn_date" placeholder="Select Date" required="" autocomplete="off" onfocus="blur()" />
                        </div>  
                        <div class="clearfix"></div><br>
                        <div class="col-md-2">Credit Note Document </div>
                        <div class="col-md-4"><input type="file" name="cn_letter_file" id="cn_letter_file" class="form-control input-sm cn_letter_file" required="" /></div>
                        <?php }else{ ?>    
                        ...
                        <input type="hidden" class="form-control input-sm cn_no" name="cn_no" id="cn_no" value="" />
                        <input type="hidden" class="form-control input-sm cn_date" data-provide="datepicker" name="cn_date" id="cn_date" value="" />
                        
                    <?php }
                    if($doa_return_type==2){       
                        
                        $idbrand = $this->input->post('idbrand');
                        $brands = $this->General_model->get_brand_byid($idbrand); ?>
...
//      save sales return
        ...
        $old_landing = $this->input->post('old_landing');
         $sales_details = $this->common_model->getSingleRow('sale',array('id_sale'=>$id_sale));
        $data = array(
            'date' => $date,
            'idsale' => $id_sale,
            'doa_id' => $doa_id,
...
//            die($total_qty);
            ...
            $idsale_productt = $selected_sale_products[0];
            if($erp_type==0){
                $idsale_productt='0000';
            }
                ...
                $sale_product = array(
                    ...
                    'idsale_product' => $idsale_productt,
...
//        die('<pre>'.print_r($_POST,1).'</pre>');
        ...
        $cn_date = $this->input->post('cn_date');
        ...
        $idservice_excecutive = $this->input->post('idservice_excecutive');
...
//      save sales return
        ...
        if($doa_return_type==2){            
            ...
            $inward_request = array(
                ...
                $imei_history[]=array(
                'imei_no' => $imeino,
                'entry_type' => 'Replace/Upgrade ('.$new_imeis.') - DOA Closure with new Handset ('.$new_enter_imei.')',
                'entry_time' => $return_date,
                'date' => $date,
                'idbranch' => $idbranch,
                'idgodown' => 3,
            ...
            $ary_service = array(
                    ...
                    'process_status' => 19,
                    'warranty_status' => 4,
                    'closure_remark' => 'Replace/Upgrade - Closed - Received New Handset',
                    'local_to_branch' => $return_date,
                    'sales_return_invid' => $sales_return_invid,
                    'procesed_by_excecutive' => $date
            );            
        }elseif($doa_return_type==4){           
            $prodlink = 'assets/doa_letter_file/';
            $config = array(
            'image_library' => 'gd2',
            'upload_path' => $prodlink,
            'allowed_types' => 'jpg|jpeg|gif|png|jfif|pdf',
            'file_name' => $_FILES['cn_letter_file']['name'],
            'maintain_ratio' => TRUE,
            'create_thumb' => TRUE,
            );
            $this->load->library('upload',$config);
            $this->upload->initialize($config);
            if($this->upload->do_upload('cn_letter_file')){
                $uploadData = $this->upload->data();
                $img = $uploadData['file_name'];
            }else{
                $img = NULL;
            ...
            $id_sale = $this->input->post('id_sale');
                ...
                $imei_history[]=array(
                'imei_no' => $imeino,
                'entry_type' => 'Replace/Upgrade ('.$new_imeis.') - DOA Closure with Credit Note ('.$cn_no.')',
                'entry_time' => $return_date,
                'date' => $date,
                'idbranch' => $idbranch,
                'idgodown' => 3,
            ...
            $ary_service = array(
                    'closed' => $date,
                    'process_status' => 11,
                    'closure_remark' => 'Replace/Upgrade - Closed - Received CN',
                    'local_to_branch' => $return_date,
                    'sales_return_invid' => $sales_return_invid,
                    'procesed_by_excecutive' => $date,
                    'doa_letter_path' => $img,
            ...
            $ary_service = array(
                    ...
                    'process_status' => 17,
                    'doa_letter_path' => $img,
                    'closure_remark' => 'Replace/Upgrade -  Received DOA letter',
                    'local_to_branch' => $return_date,
                    'sales_return_invid' => $sales_return_invid,
                    'procesed_by_excecutive' => $date,
                    'doa_id' => $doa_id,
            ...
            $doa_reconciliation = array(   
                ...
                $imei_history[]=array(
                'imei_no' => $imeino,
                'entry_type' => 'Replace/Upgrade ('.$new_imeis.') -  Received DOA letter',
                'entry_time' => $return_date,
                'date' => $date,
                'idbranch' => $idbranch,
                'idgodown' => 3,
            ...
            $ary_service = array(
                    'closed' => $date,
                    'process_status' => 3,
                    'closure_remark' => 'Force DOA',  
                    'force_doa_date' =>    $return_date,
                    'local_to_branch' => $return_date,                    
                    'sales_return_invid' => $sales_return_invid,
                );
            $imei_history[]=array(
                'imei_no' => $imeino,
                'entry_type' => 'Replace/Upgrade ('.$new_imeis.') - Force DOA',
                'entry_time' => $return_date,
                'date' => $date,
                'idbranch' => $idbranch,
                'idgodown' => 3,
...
//            die($total_qty);
            ...
            $selected_row_tax = $this->input->post('tax');
                ...
                $idsalereturnproduct = $this->Sales_return_model->save_sales_return_product($sale_product);
                 ...
                 if($doa_return_type!=2 && $doa_return_type!=4){
                        $update_stock = array(                    
                        'created_by' => $sales_return_by,
                        'sales_return_by' => $sales_return_by,
                        'return_date' => $return_date,
...
//        die('<pre>'.print_r($parr,1).'</pre>');
        ...
        for($i = 0; $i < count($idvariant); $i++){
            ...
            if($skutype[$i] == 4){ //qunatity
                ...
                $imei_history[]=array(
                    'imei_no' => $imei[$i],
                    'entry_type' => 'Sale - Replaced againts DOA - '.$imeino,
                    'entry_time' => $datetime,
                    'date' => $date,
                    'idbranch' => $idbranch,
                    'idgodown' => $idgodown[$i],
...
//        die('<pre>'.print_r($_POST,1).'</pre>');
        ...
        if($this->upload->do_upload('doa_letter_file')){
            ...
            $id_sale = $this->input->post('id_sale');
                ...
                $imei_history[]=array(
                'imei_no' => $imeino,
                'entry_type' => 'Force DOA Closure with Credit Note ('.$cn_no.')',
                'entry_time' => $return_date,
                'date' => $date,
                'idbranch' => $idbranch,
                'idgodown' => 3,
            ...
            $ary_service = array(
                    'closed' => $date,
                    'process_status' => 20,
                    'closure_remark' => 'Force DOA - Closed - Received CN',
                    'local_to_branch' => $return_date,
                    'sales_return_invid' => $sales_return_invid,
                    'procesed_by_excecutive' => $date,
                    'doa_letter_path' => $img,
        ...
        if($doa_return_type==1  || $doa_return_type==4){
            $this->Service_model->update_stock($idservice,$update_stock);            
        }else{
            $this->Service_model->delete_idservice_from_stock($idservice);
        }
    ...
    public function ajax_get_imei_details_for_doa_replace() {
        $imei = $this->input->post('imei');
        $doa_imei = $this->input->post('imei_no');
        $old_idvariant = $this->input->post('old_idvariant'); 
        $idbranch = $this->input->post('idbranch');       
        ...
        if($skuvariant){
            ...
            if(count($models)){
                $oldidvariant = $this->General_model->get_model_variant_data_byidvariant($old_idvariant);
                $idcategory=$oldidvariant->idcategory;
                if($idcategory==2 || $idcategory==28 || $idcategory==31 || $idcategory==33){
                    $variants = $this->Service_model->get_same_variants_for_doa_replace_up($oldidvariant->idmodel,1);
                }else{ 
                    $variants = $this->Service_model->get_same_variants_for_doa_replace_up($oldidvariant->id_variant,0);
                }  
                $variantss=array();
                foreach($variants as $var){
                    $variantss[] = $var->id_variant;
                }       
                $landing=$models[0]->landing;
                if(in_array($models[0]->idvariant,$variantss)){   
                   
                     $doa_sale_data = $this->Service_model->get_sale_product_byimei($doa_imei);                     
                     if(count($doa_sale_data)>0){
                        $landing=$doa_sale_data[0]->old_landing;                     
                     }
                }
                foreach($models as $model){
                    if($model->idgodown == 1 || $model->idgodown == 6){
                        ...
                        <tr class="product_tr" id="m<?php echo $model->id_stock ?>">
                            ...
                            <td>
                                <input type="hidden" id="read_skuprice<?php echo $skuvariant ?>" value="<?php echo $model->mop ?>" />
                                <input type="hidden" id="readprice<?php echo $model->id_stock ?>" value="<?php echo $model->mop ?>" />
                                <input type="hidden" id="landing" name="landing[]" class="landing" value="<?php echo $landing ?>" />
                                <input type="hidden" id="mop" name="mop[]" class="mop" value="<?php echo $model->mop ?>" />
                                <input type="hidden" id="mrp" name="mrp[]" class="mrp" value="<?php echo $model->mrp ?>" />
                                <input type="hidden" id="salesman_price" name="salesman_price[]" class="salesman_price" value="<?php echo $model->salesman_price ?>" />
                                <input type="number" id="price" class="form-control input-sm price" name="price[]" value="<?php echo $model->mop ?>" min="<?php echo $model->landing ?>" step="0.001" style="width: 90px" max="<?php echo $model->mrp ?>" />
                ...
                <?php }}else{
                    echo '3'; // Other that New/Online Godown not accepted
                }}}else{
                die('0');
            }
        }
...
//        $sale_product_data = $this->Service_model->get_sale_old_product_byidsaleproduct($product_id);
...
//        }
    ...
    public function counter_faulty_pending_approval() {
        $q['tab_active'] = '';
        $q['service_stock'] = $this->Service_model->get_service_counter_faulty_pending_stock();
        $this->load->view('service/counter_faulty_pending_approval', $q);
    }
    public function service_executive_mapping() {
        $q['tab_active'] = '';        
        $q['branch_data'] = $this->General_model->get_active_branch_service_executive();           
        $this->load->view('service/service_executive_mapping', $q);
    }    
    public function ajax_service_executive() {   
        $idbranch = $this->input->post('idbranch');            
        $active_excecutive = $this->General_model->get_user_details_byidrole(39);                
        echo '<input type="hidden"  name="id_branch" id="id_branch" value="'.$idbranch.'" />';
        echo '<select class="chosen-select form-control" name="idservice_executive" id="idservice_executive" required=""><option value="">Select Executive</option>';
        foreach ($active_excecutive as $excecutive) { 
            echo '<option  value="'.$excecutive->id_users .'">'.$excecutive->user_name.'</option>';
        }
    }
    public function update_service_executive(){
        $this->db->trans_begin();        
        $idbranch = $this->input->post('idbranch');
        $idservice_executive=  $this->input->post('idservice_executive');                
        $updateArray = array(                        
                        'idservice_executive' => $idservice_executive,
                    ); 
