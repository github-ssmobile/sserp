        ...
        if(count($payment_reconcilationdata) > 0){ ?>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php $idp = sprintf("%'.05d\n", $pr->id_payment_reconciliation);  echo date('Y', strtotime($pr->date)).''.$pr->branch_code.'/V'.$idp; ?></td>
...
//        $bank_data = $this->General_model->get_active_bank_data();
        ...
        <tbody>
            ...
            <tr>
                <td><?php echo $i++; ?></td>
                <td><?php echo date('d-m-Y', strtotime($credit->inv_date)) ?></td>
...
//        $idpayment_head = $this->input->post('idpayment_head');
        ...
        <tbody class="data_1">
            ...
            <tr>
                ...
                <td><?php echo $credit->final_total ?></td>
                
                <td><?php echo $credit->customer_fname.' '.$credit->customer_lname.' - '.$credit->customer_contact ?></td>
                <!--<td><?php // echo $credit->customer_contact ?> </td>-->
                <!--<td><?php // echo $credit->customer_gst ?> </td>-->
                <td><?php echo $credit->payment_mode ?></td>
                <?php if($credit->idpayment_mode == 4 || $credit->idpayment_mode == 29){ ?>
                <td  class="bfl_form" onclick="get_bflform(<?php echo $credit->idsale ?>)"><?php echo $credit->transaction_id ?></td>
                <?php }else{ ?>
                <td><?php echo $credit->transaction_id ?></td>
                <?php } ?>
                <td><?php echo $credit->amount ?></td>
...
//        $idpayment_head = $this->input->post('idpayment_head');
        ...
        $credit_report = $this->Reconciliation_model->ajax_get_receivables_received_report($idpayment_mode,$idbranch,$datefrom,$dateto,$modes,$branches,$date_filter);
        if(count($credit_report) == 0){
            echo '<center><h3><i class="mdi mdi-alert"></i> Entries Not Found.</h3>'.
                    '<img src="'.base_url().'assets/images/highAlertIcon.gif" />'
                . '</center>'; 
...
//            $payment_devices = $this->Reconciliation_model->get_devices_byidpayment_mode($idpayment_mode); ?>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php echo $credit->payment_mode ?></td>
                    <?php if($credit->idpayment_mode == 4 || $credit->idpayment_mode == 29){ ?>
                    <td  class="bfl_form" onclick="get_bflform_report(<?php echo $credit->idsale ?>)"><?php echo $credit->transaction_id ?></td>
                    <?php }else{ ?>
                    <td><?php echo $credit->transaction_id ?></td>
                    <?php } ?>
                    <td><?php echo $credit->amount ?></td>
                    ...
                    <td><?php echo $credit->return_count ?></td>
                </tr>
                </form>
                <?php } ?>
            </tbody>
...
//            
            $payment = array(
                ...
                'idpayment_head' => 6,
                'idpayment_mode' => 17,
                'transaction_id' => $this->input->post('utr'),
                'inv_no' => $this->input->post('inv_no'),
                'idcustomer' => $this->input->post('idcustomer'),
                'idbranch' => $this->input->post('idbranch'),
                'created_by' => $this->input->post('iduser'),
...
//        die(print_r($_POST));
    ...
    public function ajax_get_insurance_pending_reconciliation() {
        ...
        $insurance_pending_recon = $this->Reconciliation_model->get_insurance_pending_recon_byvariant_status_branch($idvariant,0, $idbranch,$branches);
...
//        $insurance_pending_recon = $this->Reconciliation_model->get_insurance_pending_recon_byvariant_status($idvariant,0);
    ...
    public function get_insurance_recon_bystatus_date() {
        ...
        $insurance_recon_data = $this->Reconciliation_model->get_insurance_recon_bystatus_date_byidbranch($idvariant, $status, $datefrom, $dateto, $sale_recon_by, $idbranch, $branches); ?>
            ...
            <table class="table table-condensed table-striped table-bordered table-hover" style="margin-bottom: 0;">
                ...
                <tbody class="data_1">
                    ...
                    <tr>
                        ...
                        <td><?php echo $stock->doa_id ?></td>
                        <td>
                        <?php if (file_exists('assets/doa_letter_file/'.$stock->doa_letter_path)) { ?>                            
                            <a class="waves-effect waves-block thumbnail text-center" target="_blank" href="<?php echo base_url('assets/doa_letter_file/'.$stock->doa_letter_path) ?>" style="color: #1b6caa; padding: 5px;"><i class="pe pe-7s-note2 fa-lg"></i> View Letter</a>
                        <?php } ?>
                        </td>
...
//        die(print_r($_POST));
    ...
    public function credit_received_edit(){
        $q['tab_active'] = '';
        $iduser = $_SESSION['id_users'];
        if($_SESSION['level'] == 1){
            $q['branch_data'] = $this->General_model->get_active_branch_data();
        ...
        $q['payment_mode'] = $this->Reconciliation_model->get_payment_modes_for_credit_received_edit_report();
         $this->load->view('reconciliation/credit_received_edit', $q);
    }
    public function ajax_get_credit_received_edit(){
//        echo '<pre>';
//        print_r($_SESSION);die;
        ...
        $modes = $this->input->post('modes');
        $branches = $this->input->post('branches');
        $payment_reconcilationdata = $this->Reconciliation_model->ajax_get_credit_received_report_byfilter_edit($idpaymentmode, $idbranch, $datefrom, $dateto, $modes, $branches);
...
//        die('<pre>'.print_r($payment_reconcilationdata,1).'</pre>');
        if(count($payment_reconcilationdata) > 0){ ?>
            <thead class="fixedelement">
                ...
                <th>Days</th>
                <th>Credit Amount</th>
                <th>Credit Received Amount</th>
                <th>Pending Credit Amount</th>
                <th>Credit Remark</th>
                <th>Edit</th>
                <?php 
                if($_SESSION['level'] == 1){
                  if($_SESSION['idrole'] == 9){ ?>
                <th>Delete</th>   
               <?php   
                  } 
                }
            ...
            <tbody>
                <?php $i=1; foreach ($payment_reconcilationdata as $pr){
                $recive_date = $pr->date;
                $rev_date = strtotime($recive_date);
                date_default_timezone_set('Asia/Kolkata');
                $date = date('Y-m-d', time());
                $ch_date = strtotime($date);
                $datediff = $ch_date - $rev_date;
                $datediff_ch = round($datediff / (60 * 60 * 24));
                ?>
                <tr>
                 <form class="credit_edit_submit_form">
                    <td><?php echo $i++; ?><input type="hidden"  value="<?php echo $pr->id_payment_reconciliation ?>"></td>
                    <td><?php echo $pr->inv_no ?></td>
                    <td><?php echo $pr->invoice_date ?></td>
                    <td><?php echo $pr->date ?></td>
                    <td><?php echo $pr->branch_name ?></td>
                    <td><?php echo $pr->payment_mode ?></td>
                    <td>
                        <div class="myDiv1" id="div1_<?php echo $pr->id_payment_reconciliation ?>" style="display: none"><input type="text" name="txn_id" id="<?php echo 'txn_'.$pr->id_payment_reconciliation; ?>" class="txn_id form-control input-sm" value="<?php echo $pr->transaction_id; ?>" /></div>
                        <div class="txn_id myDiv2" id="div2_<?php echo $pr->id_payment_reconciliation ?>"><?php echo $pr->transaction_id; ?></div>
                        <input type="hidden" id="txnold_<?php echo $pr->id_payment_reconciliation ?>" name="txnold_<?php echo $pr->id_payment_reconciliation ?>" value="<?php echo $pr->transaction_id ?>">
                        <input type="hidden" id="invno_<?php echo $pr->id_payment_reconciliation ?>" name="invno_<?php echo $pr->id_payment_reconciliation ?>" value="<?php echo $pr->inv_no ?>">
                        <input type="hidden" id="saleid_<?php echo $pr->id_payment_reconciliation ?>" name="saleid_<?php echo $pr->id_payment_reconciliation ?>" value="<?php echo $pr->idsale ?>">
                        <input type="hidden" id="reconlid_<?php echo $pr->id_payment_reconciliation ?>" name="reconlid_<?php echo $pr->id_payment_reconciliation ?>" value="<?php echo $pr->id_payment_reconciliation ?>">
                        <input type="hidden" class="user_id" name="user_id" value="<?php echo $_SESSION['id_users'] ?>">
                    </td>
                    <td><?php echo $pr->customer_fname.' '.$pr->customer_lname ?></td>
                    <td><?php echo $pr->customer_contact ?></td>
                    <td><?php echo $pr->final_total ?></td>
                    ...
                    <td><?php 
                        ...
                        echo round($datediff / (86400)); 
                    ?></td>
                    <td><?php echo $pr->sale_amnt ?></td>
                    <td><?php echo $pr->payment_rec ?></td>
                    <td><?php 
                    $pending_amnt = $pr->sale_amnt - $pr->payment_rec;
                    echo $pending_amnt;
                    ?></td>
                    <td><?php echo $pr->credit_receive_remark ?></td>
                    <td>
                        ...
                        if($datediff_ch <= 4){
                        $transactionid_update = $this->Reconciliation_model->get_transaction_update_count($pr->id_payment_reconciliation,$pr->idsale,$pr->inv_no);    
                        if($transactionid_update <=0){
                        ?>
                        <div class="myDiv1" id="button2_<?php echo $pr->id_payment_reconciliation ?>" style="display: none"><button type="button" class="btn btn-sm btn-primary waves-effect waves-ripple save" name="id_model"  style="margin: 0; text-transform: capitalize" onclick="submit_edit_credit(<?php echo $pr->id_payment_reconciliation ?>,event)">Submit</button></div>
                        <div class="myDiv2" id="button1_<?php echo $pr->id_payment_reconciliation ?>"><a class="hide-btn btn btn-outline-info btn-sm waves-effect waves-ripple" style="margin: 0; text-transform: capitalize"> Edit</a></div>
                       <?php   
                           }
                         }
                        }else{ ?>
                  ...
                  if($_SESSION['level'] == 1){
                  if($_SESSION['idrole'] == 9){ ?>
                    <td><div class="myDiv3" id="button3_<?php echo $pr->id_payment_reconciliation ?>"><button type="button" class="btn btn-sm btn-danger waves-effect waves-ripple save" name="delete_sale"  style="margin: 0; text-transform: capitalize" onclick="submit_delete_credit(<?php echo $pr->id_payment_reconciliation ?>,event)">Delete</button></div></td>
                   <?php   
                   } 
                  }
                  ?> 
    ...
    public function update_transactionid(){
        $this->Reconciliation_model->update_transactionid();    
    }
    public function delete_credit_transactionid() {
        $this->Reconciliation_model->delete_credit_transactionid();   
    }
    public function receivables_received_edit(){
        $q['tab_active'] = '';
        $iduser = $_SESSION['id_users'];
        if($_SESSION['level'] == 1){
            $q['branch_data'] = $this->General_model->get_active_branch_data();
        ...
        $this->load->view('reconciliation/receivables_received_edit', $q);
    }
    public function ajax_receivables_received_edit() {
//        echo '<pre>';
//        die(print_r($_POST));
        $idbranch = $this->input->post('idbranch');
//        $idpayment_head = $this->input->post('idpayment_head');
...
//            $payment_devices = $this->Reconciliation_model->get_devices_byidpayment_mode($idpayment_mode); ?>
            ...
            <tbody>
                <?php $i=1; foreach ($credit_report as $credit) { ?>
                <form class="ajax_recon_form">
                <tr id="reconsilationrow_<?php echo $credit->id_payment_reconciliation ?>">
                    <!--<td><?php // echo $i++; ?></td>-->
                    <td><?php echo date('d-m-Y', strtotime($credit->date)) ?></td>
                    <td><?php echo $credit->branch_name ?></td>
                    <td><a href="<?php echo base_url('Sale/sale_details/'.$credit->idsale) ?>" target="_blank" style="color: #3333ff"><?php echo $credit->inv_no ?></a></td>
                    ...
                    <td><?php echo $credit->user_name ?></td>
                    <td>
                        <div class="myDiv2" id="button_<?php echo $credit->id_payment_reconciliation ?>">
                            <a class="hide-btn btn btn-outline-info btn-sm waves-effect waves-ripple" id="returnbtn_<?php echo $credit->id_payment_reconciliation ?>" onclick="payment_reconciliation_return(<?php echo $credit->id_payment_reconciliation ?>)" style="margin: 0; text-transform: capitalize">Revised<span>&nbsp;<i class="mdi mdi-keyboard-return fa-lg"></i></span></a>
                        </div>
                        <input type="hidden" id="returncount_<?php echo $credit->id_payment_reconciliation ?>" value="<?php echo $credit->return_count ?>" >
                        <input type="hidden" id="idsalepayment_<?php echo $credit->id_payment_reconciliation ?>" value="<?php echo $credit->idsale_payment ?>" >
                        <input type="hidden" id="idpaymenthead_<?php echo $credit->id_payment_reconciliation ?>" value="<?php echo $credit->idpayment_head ?>" >
                    </td>
                </tr>
                </form>
                <?php } ?>
        ...
        <?php }}else{ 
        $idbranches = $this->input->post('branches');
        $cash_deposit= $this->Reconciliation_model->ajax_get_cheque_reconciled_edit($idbranch,$idbranches,$datefrom,$dateto,2);
//        echo '<pre>';
//        print_r($cash_deposit);die;
        if(count($cash_deposit) == 0){
            echo '<center><h3><i class="mdi mdi-alert"></i> Entries Not Found.</h3>'.
                '<img src="'.base_url().'assets/images/highAlertIcon.gif" />'
            . '</center>'; 
        ...
        <tbody>
            <?php $i=1; foreach ($cash_deposit as $deposit) { ?>
            <form class="ajax_recon_form">
            <tr id="reconsilationrow_<?php echo $deposit->id_payment_reconciliation ?>">
                <td><?php echo date('d-m-Y', strtotime($deposit->date)) ?></td>
                <td><?php echo $deposit->branch_name ?></td>
                ...
                <td>
                    <div class="myDiv2" id="button_<?php echo $deposit->id_payment_reconciliation ?>">
                        <a class="hide-btn btn btn-outline-info btn-sm waves-effect waves-ripple" id="returnbtn_<?php echo $deposit->id_payment_reconciliation ?>" onclick="payment_reconciliation_return(<?php echo $deposit->id_payment_reconciliation ?>)" style="margin: 0; text-transform: capitalize">Revised<span>&nbsp;<i class="mdi mdi-keyboard-return fa-lg"></i></span></a>
                    </div>
                    <input type="hidden" id="returncount_<?php echo $deposit->id_payment_reconciliation ?>" value="<?php echo $deposit->return_count ?>" >
                    <input type="hidden" id="idsalepayment_<?php echo $deposit->id_payment_reconciliation ?>" value="<?php echo $deposit->idsale_payment ?>" >
                    <input type="hidden" id="idpaymenthead_<?php echo $deposit->id_payment_reconciliation ?>" value="<?php echo $deposit->idpayment_head ?>" >
                </td>
            </tr>
            </form>
            <?php } ?>
    ...
    public function payment_reconciliation_return(){
     $this->db->trans_begin();
     $return_id = $this->Reconciliation_model->payment_reconciliation_return();
     if ($this->db->trans_status() === FALSE){
        $this->db->trans_rollback();
        $q['result'] = 'Failed';
    }else{
    ...
    public function ajax_bfl_data_form() {

     $this->db->trans_begin();
     $bfl_data = $this->Reconciliation_model->ajax_bfl_data_form();
     if ($this->db->trans_status() === FALSE){
        $this->db->trans_rollback();
