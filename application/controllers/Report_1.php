...
<!--            <tr>
    ...
    public function ajax_new_daybook_report1() {
        $datefrom = $this->input->post('datefrom');
        $dateto = $this->input->post('dateto');
        $idbranch = $this->input->post('idbranch');
        $branches = $this->input->post('branches');
        ...
        $daybook_credit_buyback_recieve_report = $this->Report_model->ajax_get_daybook_credit_buyback_recieve_report($datefrom,$dateto,$idbranch,$branches);
        // die(print_r($daybook_expense_report));
        $amt=array();$ssramt=array();$amt[0]=0;$ssramt[0]=0;$sramt=0;$expamt=0;$depositamt=0;$cppramt=0; ?>
        <thead class="fixedelement">
            <th>Date</th>
            <th>Branch</th>
            <th>Invoice</th>
        ...
        <tbody>
            ...
            <tr>
                <td><?php echo $cprrdaybook->date ?> </td>
                <td><?php echo $cprrdaybook->branch_name ?> </td>
                <td><a style="color: #3333ff" target="_blank" href="<?php echo base_url('Payment/cash_payment_receive_print/'.$cprrdaybook->id_cash_payment_receive) ?>">Receipt/<?php echo $cprrdaybook->id_cash_payment_receive ?></a></td>
                <td><?php echo $cprrdaybook->amount; $cppramt = $cppramt + $cprrdaybook->amount; ?> </td>
                <td colspan="<?php echo $payment_mode_count + 4; ?>"></td>
            </tr>
            <?php } ?> 
            <tr>
                <th></th><th></th>
                <th>Total</th>
                <th><?php echo $cppramt; ?></th>
                <th colspan="<?php echo $payment_mode_count + 4; ?>"></th>
            </tr>
            <?php }else{ ?>
            <tr>
                <td colspan="<?php echo $payment_mode_count + 4; ?>"><h5 class="red-text"><i class="mdi mdi-alert"></i> Cash Payment Received Record Not Found...</h5></td>
            ...
            <tfoot class="fixedelement_bottom">
                <th></th><th></th>
                <th>Available Cash</th>
                <th><?php $total_cash = $opening_cash_bydate->daybook_cash  + $amt[0] + $ssramt[0] - $sramt - $expamt - $depositamt; echo $total_cash; ?></th>
                <?php for($i=1; $i < count($mode_name); $i++){ 
                    $sale_amount = 0;$creditr_amount = 0;
                    if (array_key_exists($i, $amt)){
                        $sale_amount = $amt[$i];
                    }if (array_key_exists($i, $ssramt)){
                        $creditr_amount = $ssramt[$i];
                    } ?>
                <th><?php echo $sale_amount + $creditr_amount; ?></th>
                <?php } ?>
                <th><?php echo array_sum($amt) + array_sum($ssramt); ?></th>
            </tfoot>
        </tbody>
    <?php 
    }
...
//                       $total_sett_amt =  $sale->total_settlement;
                    ...
                    <td><?php echo round($dis_amt,2); ?></td>   
