...
//                    if($auth['status'] == 200){
                        ...
                        $response = $this->Api_Model->get_customer_bycontact($cust_contact,$idbranch);
                        
                        $response['var_closer']=$this->verify_cash_closure($idbranch);
//                        die(print_r($response));
                        if(count($response['customer'])==0){            
                            $resp= array('status' => 204,'message' => 'Customer not found.', 'data' => $response );
                        } else { 
                            $resp= array('status' => 200,'message' => 'Verification successfull', 'data' => $response );
                        }
                ...
                $auth = $this->Api_Model->auth();                    
                    if($auth['status'] == 200){
                           $result=array(); 
                            ...
                            $userdata = $this->General_model->get_user_all_data_byid($users_id);                              
                            $idbranch=$userdata[0]->idbranch;
                            ...
                            $credit_days = $invoice_no->credit_days;
                            ...
                            if($head_status == 1){ //Branch has credit/custudy payment head
                                if($credit_status == 1){ //display credit/custudy payment head 
                                    $head_data = $this->General_model->get_branch_has_paymenthead_byidbranch($idbranch);
                                }else{
                                    ...
                                    $head_data = $this->General_model->get_active_payment_head_by_headids($ids);    
                                }
                            }else{
                                $head_data = $this->General_model->get_branch_has_paymenthead_byidbranch($idbranch);
                            }
            ...
            if ($auth['status'] == 200) {
                ...
                if ($skuvariant) {
                    ...
                    if (count($token_data) > 0) {
                        ...
                        $models['stock'] = $this->Api_Model->ajax_stock_data_byimei_branch($imei, $idbranch,$idgodown);
    ...
    public function save_sale_token() {
        ...
        if ($check_auth_client == true) {
            ...
            if ($auth['status'] == 200) {                     
                ...
                $bfl_dataa = $this->input->post('bfl_data');
                 $bfldata = json_decode($bfl_dataa);
                // BFL integration
                 
                 
                if($this->input->post('bfl_data')=="null" || $this->input->post('bfl_data')==null){     }else{
                    
                    if($bfldata->CustomerPhoneNo!=$customer_data->customer_contact){
                        $customer_data = $this->Sale_model->ajax_get_customer_bycontact($bfldata->CustomerPhoneNo);
                        if(count($customer_data)>0){
                             $updata = array(
                            'customer_fname' => $bfldata->CustomerFirstName,
                            'customer_lname' => $bfldata->CustomerLastName,
                            'customer_contact' => $bfldata->CustomerPhoneNo,
                            'customer_gst' => $customer_data->customer_gst,
                            'customer_pincode' => $bfldata->PinCode,
                            'customer_city' => $bfldata->CITY,
                            'customer_district' => $bfldata->CITY,
                            'idstate' => $customer_data->id_state,
                            'customer_state' => $bfldata->STATE,
                            'customer_address' => $bfldata->AddressLine1.", ".$bfldata->AddressLine2.", ".$bfldata->AddressLine3.", ".$bfldata->Area.", ".$bfldata->Landmark,
                            'entry_time' => $datetime,                    
                            );
                            $this->General_model->edit_customer_byid($customer_data->id_customer, $updata);
                            $customer_data = $this->Sale_model->get_customer_byid($customer_data->id_customer)[0];
                        }else{                        
                            $data = array(
                               'customer_fname' => $bfldata->CustomerFirstName,
                               'customer_lname' => $bfldata->CustomerLastName,
                           ...
                           $idcustomer = $this->Sale_model->save_customer($data);
                           if ($idcustomer) {
                               $customer_data = $this->Sale_model->get_customer_byid($idcustomer)[0];
                           }
                        }
                    }
                    $downscheme = ($bfldata->CustomerDownPayment / $bfldata->TotalEMI);
                    $loanscheme = ($bfldata->NetLoanAmount / $bfldata->TotalEMI);
...
//                Sale_product
                ...
                if($this->input->post('bfl_data')=="null" || $this->input->post('bfl_data')==null){   }else{
                    $bfl_data = array(
                            'do_id' => $bfldata->DONumber,                       
                            'bfl_brand' => $bfldata->ManufacturerName,
                            'bfl_model' => $bfldata->ModelNo,
                            'bfl_srno' => $bfldata->SerialNo,
                            'idcustomer' => $idcustomer,
                            'mobile' => $bfldata->CustomerPhoneNo,
                            'customer_name' => $bfldata->CustomerName,
                            'customer_gst' => $customer_data->customer_gst,
                            'scheme_code' => $bfldata->SchemeId,
                            'scheme' => "(".$downscheme."/".$loanscheme.")",
                            'mop' => $bfldata->InvoiceAmount,
                            'downpayment' => $bfldata->CustomerDownPayment,
                            'loan' => $bfldata->NetLoanAmount,
                            'emi_amount' => $bfldata->TotalEMI,
                            'tenure' => $bfldata->Tenure,
                            'bfl_remark' => "",
                            'idsaletoken'=> $idsaletoken,
                            'entry_time' => $datetime,
                        );
...
//                curl_close($ch);  
    ...
    function json_output($statusHeader,$response)
	{
        ...
        $this->load->view('master/model_vendor_sku',$q);        
    }
    ...
    public function verify_cash_closure($idbranch) {
