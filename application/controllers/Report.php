...
//        die(print_r($_POST));
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><?php echo $deposit->return_count ?></td>
            </tr>
            </form>
            <?php } ?>
        </tbody>
        ...
        <tbody id="myTable">
            ...
            <tr>
                ...
                <td><?php if($cash_deposit->reconciliation_status == 0){ echo "Deposited"; }else{ echo "Reconciled"; } ?></td>
                 <?php if($_SESSION['idrole'] == 9){?>
                  <td><a href="<?php echo base_url()?>Payment/delete_cash_deposite/<?php echo $cash_deposit->id_cash_deposite_to_bank?>" class="btn btn-warning btn-floating waves-effect btndelete"><i class="fa fa-trash-o"></i></a></td>
                <?php } ?>
            </tr>
...
// refund advanced payment in cash
            ...
            <tr>
                ...
                <td><a style="color: #3333ff" target="_blank" href="<?php echo base_url('Payment/advance_booking_received_receipt/'.$adv_refamt->id_advance_payment_receive) ?>">AdvRef/<?php echo $adv_refamt->branch_code.'/'.$adv_refamt->id_advance_payment_receive ?></a></td>
            ...
            <table class="table table-bordered table-condensed" id="credit_note_report">
                ...
                <tbody class="data_1">
                    <?php $sr=1; foreach ($credit_note as $cnote){ 
                       ...
                       $gst_type = $cnote->gst_type; $igst=0;$cgst=0;$sgst=0;
                       if ($gst_type == 1) {
                            $igst = $cnote->igst_per;
                        }else{                             
                            $cgst = $cnote->cgst_per;
...
//        die('<pre>'.print_r($sale_data,1).'</pre>');
    ...
    public function ajax_get_tally_jio_router_sale(){
        ...
        $sale_data = $this->Sale_model->get_jio_tally_sale_report($from, $to, $idcompany, $dc); ///new///
        $payment_mode_data = $this->General_model->get_active_payment_mode_data();
...
//        die('<pre>'.print_r($sale_data,1).'</pre>');
        ...
        <table id="sale_report" class="table table-bordered table-striped table-condensed table-info">
            ...
            <tbody class="data_1">
                <?php  $sr=1; $totalround = 0; $total =0; $total_base=0;$total_cgst=0;$total_sgst=0;$total_igst=0;$cmobine=0; $saleid=0; $sper=0;
                $old_inv=null;
                $total_sett_amt =0;
                ...
                foreach ($sale_data as $sale) { 
                    ...
                    if($sale->gst_type == 1){
                        ...
                        $sper = $sale->cgst_per + $sale->sgst_per;
                    }
                    ?>
                ...
                <tr>
                    <td><?php echo $sr++; ?></td>
                    <td><?php echo $sale->inv_no;  ?></td>
                    <td><?php echo 'Sales'; echo $sper.'%' ?></td>
                    <td><?php echo $sale->hsn?></td>
                    <td><?php echo date('d/m/Y', strtotime($sale->entry_time)) ?></td>
                    <td><?php echo $sale->customer_fname.' '.$sale->customer_lname ?></td>
                    <td><?php echo $sale->customer_contact ?></td>
                    ...
                    <td><?php echo $sale->cgst_per ?></td>    
                     <?php 
                       ...
                       if($final_total!=0){
                        $per = ($total_amount/$final_total)*100;                       
                       }else{
                           $per = 0;
                       }
...
//        die('<pre>'.print_r( $q['outward_data'],1).'</pre>');
    ...
    public function token_cancellation_report() {
        $q['tab_active'] = 'Report';
        if($_SESSION['level'] == 1){
            $q['branch_data'] = $this->General_model->get_active_branch_data();
        }elseif($_SESSION['level'] == 3){
        ...
        $this->load->view('report/token_cancellation_report', $q);
    }
    
    public function ajax_get_token_cancellation_report(){
        $from = $this->input->post('from');
        $to = $this->input->post('to');
        $idbranch = $this->input->post('idbranch');
        $branches = $this->input->post('branches');
        $idstatus = $this->input->post('idstatus');
        $tokendata = $this->Report_model->get_token_cancellation_report_data($from, $to, $idbranch, $branches, $idstatus);
        if($tokendata){ ?>
           <table class="table table-bordered table-condensed" id="token_data_report">
               <thead style="background-color: #99ccff">
                    <th>SR.</th>
                    <th>Date</th>
                    <th>Token Id</th>
               ...
               <tbody class="data_1"> 
                   <?php $sr=1; foreach ($tokendata as $tdata){
                       $cdate = date('d');
                       $token_date = date('d', strtotime($tdata->date));
                       $pending_days = $cdate - $token_date;
                       ?>
                   <tr>
                       <td><?php echo $sr++; ?></td>
                       <td><?php echo $tdata->date; ?></td>
                       <td><?php echo $tdata->idsaletoken; ?></td>
                       <td><?php echo $tdata->branch_name; ?></td>
                       <td><?php echo $tdata->inv_no;?></td>
                       <td><?php echo $tdata->full_name; ?></td>
                       <td><?php echo $tdata->qty; ?></td>
                       <td><?php echo $tdata->total_amount; ?></td>
                       <td><?php echo $tdata->customer_fname.' '.$tdata->customer_lname; ?></td>
                       <td><?php echo $tdata->customer_contact; ?></td>
                       <td><?php echo $tdata->user_name; ?></td>
                       <td><?php if($tdata->status == 0){ echo 'Pending'; } elseif ($tdata->status == 1){ echo 'Completed'; }else{ echo 'Cancelled';}  ?></td>
                       <td><?php if($tdata->status == 0){ echo $pending_days.' days'; }?></td>
                       <td><?php  echo $tdata->cancel_remark; ?></td>
                   </tr>
                   <?php } ?>
               </tbody>
           </table>
    ...
    public function daybook_summary_report() {
        $q['tab_active'] = 'Reports';
        $iduser = $_SESSION['id_users'];
        if($_SESSION['level'] == 1){
            $q['branch_data'] = $this->General_model->get_active_branch_data();
        ...
        $this->load->view('report/daybook_summary_report', $q);
    }
      public function ajax_new_daybook_summary_report() {
        $datefrom = $this->input->post('datefrom');
        $dateto = $this->input->post('dateto');
        $idbranch = $this->input->post('idbranch');
        $branches = $this->input->post('branches');
        $payment_mode_data = $this->General_model->get_active_payment_mode_data();
        $payment_head_data = $this->General_model->get_active_payment_head();
        $payment_mode_count = count($payment_mode_data);
        // sale report
        $daybook_report = $this->Report_model->ajax_get_summary_daybook_sale_report($datefrom,$dateto,$idbranch,$branches);
        // opening cash till date
        $opening_cash_bydate = $this->Report_model->ajax_get_opening_cash_bydate($datefrom,$idbranch,$branches);
        // ajax_get_cash_payment_receive_report
        $cash_payment_receive_report = $this->Report_model->ajax_get_cash_payment_receive_report($datefrom,$dateto,$idbranch,$branches);
        // ajax_get_advanced_payment_receive_report
        $adv_payment_receive_report = $this->Report_model->ajax_get_adv_payment_receive_summary_report($datefrom,$dateto,$idbranch,$branches);
        // ajax_get_advanced_payment_refund_report
        $adv_payment_refund_report = $this->Report_model->ajax_get_adv_payment_refund_report($datefrom,$dateto,$idbranch,$branches);
//        die(print_r($adv_payment_refund_report));
        // opening sales return cash
        $daybook_salesreturn_report = $this->Report_model->ajax_get_daybook_sales_return_summary_report($datefrom,$dateto,$idbranch,$branches);
        // expense cash
        $daybook_expense_report = $this->Report_model->ajax_get_daybook_expense_report($datefrom,$dateto,$idbranch,$branches);
        // bank deposit cash
        $daybook_deposite_to_bank_report = $this->Report_model->ajax_get_cash_deposite_to_bank_report($datefrom,$dateto,$idbranch,$branches);
        ...
        <thead class="fixedelement">
            ...
            <th>Product Name</th>
            <th>Invoice</th>
            <?php foreach ($payment_head_data as $payment_mode) { ?>
            <th><?php echo $payment_mode->payment_head ?> </th>
            <?php $mode_name[] = $payment_mode->payment_head; } ?>
            <th>Total</th>
        </thead>
        <tbody>
            <tr>
...
// Sale Daybook
            ...
            <tr>
                <td><?php echo $daybook->date ?> </td>
                <td><?php echo $daybook->branch_name ?> </td>
                <td><?php echo $daybook->product_name ?> </td>
                <td><a style="color: #3333ff" target="_blank" href="<?php echo base_url('Sale/sale_details/'.$daybook->idsale) ?>"><?php echo $daybook->inv_no ?></a></td>
                <?php for($i=0; $i < count($mode_name); $i++){ ?>
                <td><?php $mdn = $mode_name[$i]; echo $daybook->$mdn; ?> </td>
                <!--<td><?php // $mdn = $mode_name[$i]; $sum_amount[] += $daybook->$mdn; echo $daybook->$mdn; ?> </td>-->
...
// Sales return Daybook
            if(count($daybook_salesreturn_report) > 0){ ?>
            <tr class="bg-danger" style="color: #AC3F34">
                <td colspan="2"><h4 style="margin: 0;font-family: Kurale"><i class="mdi mdi-minus-circle-outline"></i> Sales Return</h4></td>
            ...
            <tr>
                <td><?php echo $srdaybook->date; ?> </td>
                <td><?php echo $srdaybook->branch_name ?> </td>
                <td><?php echo $srdaybook->return_product ?> </td>
                <!--<td><?php // echo $srdaybook->inv_no.'<hr style="margin:2px">SR'.$srdaybook->inv_no; ?> </td>-->
                <td><a style="color: #3333ff" target="_blank" href="<?php echo base_url('Sales_return/sales_return_details/'.$srdaybook->id_salesreturn) ?>"><?php echo $srdaybook->sales_return_invid; ?></a></td>
                <td><?php echo $srdaybook->final_total; $sramt = $sramt + $srdaybook->final_total; ?></td>
                <?php for($i=1; $i < count($mode_name); $i++){ echo '<td></td>'; } ?>
