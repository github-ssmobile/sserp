...
//            $q['product_category'] = $this->General_model->get_product_category_by_user($user_id);
    ...
    public function ajax_model_variants_allocation_data(){
        ...
        if($idproductcategory==1){            
            if($idcategory==2 || $idcategory==28 || $idcategory==31 || $idcategory==33 || $idcategory==38){
                $variants = $this->Allocation_model->get_same_variants_for_allocation($variantid,$warehouse,$idproductcategory,$idbrand,$idgodown,$allocation_type,$modelid,1);
            }else{
                $variants = $this->Allocation_model->get_same_variants_for_allocation($variantid,$warehouse,$idproductcategory,$idbrand,$idgodown,$allocation_type,$modelid,0);
            }   
...
//        die(print_r($variants));
        ...
        foreach ($branch_data as $branch){ 
            ...
            <tr>
				...
				<td class="fixleft3" style="background: #ffcccc;"><?php if($branch->brand_promoter > 0){ echo 'Yes'; }else{ echo 'No'; } ?></td>                
            <?php    
...
//        die('<pre>'.print_r($_POST,1).'</pre>');   
    ...
    public function ajax_route_allocation_data(){
        ...
        $wr = $this->Allocation_model->get_warehouse_stock_data(0,0,$idproductcategory,$idbrand,$idgodown,$warehouse,$allocation_type);
...
//        die('<pre>'.print_r($_POST,1).'</pre>');   
    ...
    public function ajax_get_branch_allocation_header(){
        ...
        ?>
        <thead class="fixedelementtop" style="text-align: center;position: none !important;">
            <th colspan="5" class="textalign"><?php echo $warehouse_name;?></th>
                ...
                foreach ($warehouse_data as $data) {                     
                    array_push($_SESSION['variant'], $data->id_variant.'-'.$data->id_godown);
                        ...
                        <tr>                  
                            ...
                            $available = ($ho_stock_qty - (($allocated_qty - $data->callocated_qty)));
                            $stock=(($data->stock_qty)+($data->intra_stock_qty));
                            ?>
                            ...
                            <td><?php echo (($data->norm_qty)-$stock) ?></td>
    ...
    public function ajax_variants_by_id(){
            ...
            $warehouse =$this->session->userdata('idbranch');
            $warehouse_data = $this->Allocation_model->get_warehouse_stock_data($variantid,$modelid,$idproductcategory,$idbrand,$idgodown,$warehouse,$allocation_type);
            ...
            ?> 
                ...
                foreach ($warehouse_data as $data) { ?>                
                        <tr>                  
                            ...
                            <td><?php echo $allocated_qty; ?></td>                                                        
                            <td><?php echo $available; ?><input type="hidden" name="<?php echo $full_name; ?>" value="<?php echo (($available==null)?0:$available); ?>" /></td>
                            <td><?php echo $model_data[0]->norm_qty; ?></td>
                            <td><?php echo $stock; ?></td>
                            <td><?php echo $days; ?></td>
                            <td><?php echo $model_data[0]->sale_qty; ?></td>
                            <td><?php echo (($model_data[0]->norm_qty)-$stock) ?></td>
...
//         die('<pre>'.print_r($_POST,1).'</pre>');   
    ...
    <div class="" style="padding: 0; margin: 0">
        <table id="model_data" class="table table-bordered table-condensed table-full-width table-responsive table-hover" style="font-size: 14px; margin: 0">
            <?php if($stock_allocation[0]->status == 0 || $stock_allocation[0]->status == 1){ ?>
            <thead class="bg-info">
                <th>Id</th>
    ...
    public function allocation_report(){
        ...
        $q['stock_allocation'] = $this->Allocation_model->get_stock_allocation_by_status_idbranch_date('','','','',$idwarehouse);
