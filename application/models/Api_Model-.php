...
class Api_Model extends CI_Model {
    ...
    public function login($username,$password,$session)
    {
        ...
        if(count($q['login'])==0){            
            return array('status' => 204,'message' => 'Username not found.', 'data' => ""  );
        } else {                
...
//		die($this->db->last_query());				
    ...
    public function get_payment_summary_byidpromoter_date($idpromotor, $idbranch,$date) {
                ...
                $this->db->join("(select pr.idpayment_head,sum(pr.amount) as amt from payment_reconciliation pr,sale WHERE sale.idsalesperson='$idpromotor' and pr.idbranch='$idbranch'  and  pr.idsale = sale.id_sale and pr.date = '$date' group by pr.idpayment_head ) pr_data", 'pr_data.idpayment_head = payment_head.id_paymenthead', 'left');
        return  $this->db->where('payment_head.active', 1)->get('payment_head')->result();    
						
    }
    
    
    public function get_drr_promotor_sale_report_slab_byidpromoter($from,$from_slab,$idslab,$idpcat, $allpcats, $idbranch,$idpromoter){
...
//       die(print_r($str));
    ...
    public function book_create_data($data)
    {
        ...
        return array('status' => 'false','message' => 'Data has been created.');
    }

    public function book_update_data($id,$data)
    {
        $this->db->where('id',$id)->update('books',$data);
        return array('status' => 'false','message' => 'Data has been updated.');
    }

    public function book_delete_data($id)
    {
        $this->db->where('id',$id)->delete('books');
        return array('status' => 'false','message' => 'Data has been deleted.');
    }
