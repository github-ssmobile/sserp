...
///////// SERVICE - VINAYAK P   /////////////
    ...
    public function get_service_details_byimei($imei){
        $this->db->select("ss.*,service_process_status.*")->where('ss.imei', $imei)     
                        ->where('ss.process_status=service_process_status.id')->from('service_process_status');                        
        return  $this->db->get('service_stock ss')->result();
    }
...
//        $this->db->select("ss.*,mv.full_name,b.*,u.user_name,br.brand_name,pc.product_category_name,ps.delivery_status,stp.*");
        ...
        if($status){
            $ar=array($status,2);
            $this->db->where_in('process_status', $ar);
        }
        if($idbrand){
            $this->db->where('ss.idbrand', $idbrand);
        }
        ...
        $this->db->where('ss.idvariant=mv.id_variant')->from('model_variants mv')
                ...
                ->where('ss.process_status=ps.id')->from('service_process_status ps')
//                ->where('stp.idservice_stock=ss.id_service')->from('service_transfer_product stp')
                ->join('users u','u.id_users=ss.idservice_excecutive', 'left');
        return $this->db->get('service_stock ss')->result();
                die(print_r($this->db->last_query()));
    }
...
//                ->where('ss.process_status=ps.id')->from('service_process_status ps')
    ...
    public function get_same_variants_for_doa_replace_up($variantid,$type) { 
        $qy="";
        if($type==0){
            ...
            $qy="select mv.id_variant from model_variants mv "               
                . "where mv.idmodel=$variantid  and active=1  group by mv.id_variant ORDER BY FIELD(mv.id_variant, $variantid) DESC";
        } 
        $query = $this->db->query($qy);   
        return $query->result();  
    }
    public function get_sale_product_byimei($imei){       
            return $this->db->select('landing as old_landing')->where('imei_no', $imei) 
                            ->get('sale_product')->result();
