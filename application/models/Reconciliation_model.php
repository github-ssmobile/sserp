        ...
        if($datefrom != '' && $dateto != ''){
            if($idpayment_mode != ''){
                if($idbranch == ''){
                        ...
                        return $this->db->select('GROUP_CONCAT(sale_product.imei_no) as imei_no,sale.final_total,payment_reconciliation.*,payment_reconciliation.date as inv_date, payment_mode.payment_mode, branch.branch_name, branch.id_branch, customer.*')
                                    ...
                                    ->join('sale_product','payment_reconciliation.idsale = sale_product.idsale','left')->join('sale','payment_reconciliation.idsale = sale.id_sale','left')
                                    ...
                                    ->group_by('sale_product.idsale,payment_reconciliation.id_payment_reconciliation')
...
//            }
    ...
    public function ajax_get_receivables_received_report($idpayment_mode,$idbranch,$datefrom,$dateto,$modes,$viewbranches,$date_filter){
//        die(print_r($_POST));
        if($idpayment_mode == ''){
            $modes_arr = explode(',',$modes);
        }else{
        ...
        if($idbranch == '' && $_SESSION['level'] == 1){
            if($datefrom != '' && $dateto != ''){
                return $this->db->select('users.user_name,payment_reconciliation.*, payment_reconciliation.date as inv_date, payment_mode.payment_mode, branch.branch_name, customer.*, payment_head.payment_head, bank.bank_name')
//                            ->where('payment_reconciliation.transfer_date between "'. $datefrom.'" and "'. $dateto .'"')
                            ->where('payment_reconciliation.'.$date_filter.' between "'. $datefrom.'" and "'. $dateto .'"')
                            ->where_in('payment_reconciliation.idpayment_mode', $modes_arr)
                            ...
                            ->where('payment_reconciliation.idcustomer=customer.id_customer')->from('customer')
                        ->where('payment_reconciliation.received_by = users.id_users')->from('users')
                            ->order_by('payment_reconciliation.id_payment_reconciliation','asc')
                            ->get('payment_reconciliation')->result();
            }elseif($datefrom == '' && $dateto == ''){
                return $this->db->select('users.user_name,payment_reconciliation.*, payment_reconciliation.date as inv_date,payment_mode.payment_mode, branch.branch_name, customer.*, payment_head.payment_head, bank.bank_name')
...
//        die(print_r($_POST));
        return $this->db->select('sale.final_total,sale.customer_fname,sale.date as invoice_date,sale.customer_lname,sale.customer_contact,payment_reconciliation.*,branch.branch_name,payment_mode.payment_mode,sale_payment.received_amount as payment_rec,sale_payment.amount as sale_amnt,branch.branch_code')                        
                        ->where('payment_reconciliation.date >=', $datefrom)
                        ->where('payment_reconciliation.date <=', $dateto)
                        ->where_in('payment_reconciliation.idpayment_mode', $modes_arr)
                        ->where_in('payment_reconciliation.idbranch', $branches)
     ...
     public function ajax_get_cash_payment_received_byidbranch($fromdate, $todate, $idbranch, $allbranches) {
        if($idbranch == 0){
            $branch = explode(',',$allbranches);
        }else{
            $branch[] = $idbranch;
        }
        return $this->db->select('cash_payment_receive.*,users.user_name,branch.branch_name')->where('cash_payment_receive.date >=',$fromdate)
                        ->where('cash_payment_receive.date <=',$todate)
                        ->where_in('cash_payment_receive.idbranch',$branch)
...
//                        ->where('apr.created_by = users.id_users')->from('users')
    ...
    public function get_advance_payment_received_report() {
        return $this->db->select('branch.branch_name,payment_head.payment_head,payment_mode.payment_mode,apr.*,users.user_name,model_variants.full_name,payment_reconciliation.payment_receive,urs.user_name as sales_person')
                        ->where('apr.idvariant = model_variants.id_variant')->from('model_variants')
                        ->where('apr.created_by = users.id_users')->from('users')
                        ->where('apr.idpayment_mode = payment_mode.id_paymentmode')
                        ->where('apr.idpayment_head = payment_head.id_paymenthead')
    ...
    public function ajax_get_advance_payment_received_report($datefrom,$dateto,$idbranch,$viewbranches,$idstatus) {
        if($idstatus == 'all'){
            $status = array(0,1);
        }else{
            $status[] = $idstatus;
        ...
        $this->db->select('branch.branch_name,payment_head.payment_head,payment_mode.payment_mode,apr.*,users.user_name,model_variants.full_name,payment_reconciliation.payment_receive,urs.user_name as sales_person');
        if($idbranch == ''){
            $branches = explode(',',$viewbranches);
        }else{
            $branches[] = $idbranch;
      ...
      if($idstatus=='all'){
                $this->db->where_in('payment_reconciliation.payment_receive', $status);
            }else if($idstatus==1){
                $this->db->where_in('payment_reconciliation.payment_receive', $status);
                $this->db->where('apr.inv_No!=""');
            }else if($idstatus==0){
            // $this->db->where_in('payment_reconciliation.payment_receive', $status);
                $this->db->where('ifnull(apr.inv_No,"N")="N"');
                $this->db->where('apr.claim',0);
            }else if($idstatus==2){
            // $this->db->where_in('payment_reconciliation.payment_receive', $status);
                $this->db->where('ifnull(apr.inv_No,"N")="N"');
                $this->db->where('apr.claim',2);
            }
// ->where_in('payment_reconciliation.payment_receive', $status)    (old line)
    ...
    public function ajax_get_doa_stock_for_recon($idvendor,$idbrand){
        ...
        return $this->db->where('(dr.status = 2 or (dr.status = 0 and dr.doa_return_type=4))')
//                        ->where('dr.doa_return_type = 1')
                        ->join('doa_stock_shipment dss','dr.iddoa_stock_shipment = dss.id_doa_stock_shipment','left')                               
                        ->where('dr.idservice = ss.id_service')->from('service_stock ss')                        
                        ->join('vendor vdr','dss.idvendor = vdr.id_vendor','left')   
...
//                        ->where('dr.doa_return_type = 1')
    ...
    public function ajax_get_doa_inward_report($idvendor,$idbrand){
        ...
        return $this->db->where('dr.status = 1')                                                
                        ...
                        ->where('dr.idservice = ss.id_service')->from('service_stock ss')                        
                        ->join('vendor vdr','dr.idvendor = vdr.id_vendor','left')       
                        ->where('ss.idvariant = model_variants.id_variant')->from('model_variants')
    ...
    public function get_insurance_pending_recon_byvariant_status_branch($idvariant,$status,$idbranch,$branchess) {
        if($idbranch == '0'){
        ...
        return $this->db->select('ir.*, users.user_name')
                        ->where('ir.sale_recon', $status)
                        ->where('ir.idvariant', $idvariant)
                        ->where_in('ir.idbranch', $branches)
                        ->join('users','ir.sale_recon_by = users.id_users','left')
                        ->get('insurance_reconciliation ir')->result();
    }
    public function get_insurance_recon_bystatus_date_byidbranch($idvariant, $status, $datefrom, $dateto, $sale_recon_by, $idbranch, $allbranch) {
        if($idbranch == '0'){
    ...
    public function update_transactionid() {
        ...
        $this->db->where('id_payment_reconciliation', $reconsilation)->update('payment_reconciliation', $data);
        
        $data1 = array(
            'idpaymentreconciliation' => $reconlid,
            'idsale' => $saleid,
            'inv_no' => $inv_no,
            'old_transaction_id' => $transactionid_old,
            'new_transaction_id' => $transactionid,
            'edited_by' => $user_id,
            );
        $this->db->insert('credit_edit_history', $data1);
    }
    public function delete_credit_transactionid() {
 
        extract($_POST);
        $this->db->trans_begin();
        $recon_amount = $this->db->select('amount,idsale_payment,inv_no')->where('id_payment_reconciliation', $reconsilation)->get('payment_reconciliation')->row();
      
        $rec_amount = $recon_amount->amount;
        $sale_id = $recon_amount->idsale_payment;
        $inv_no = $recon_amount->inv_no;
        
       $saleamount = $this->db->select('received_amount')->where('id_salepayment', $sale_id)              
                        ->where('inv_no', $inv_no)
                        ->get('sale_payment')->row();
       
       $sale_amount = $saleamount->received_amount;
       $total_amount = $sale_amount - $rec_amount;
       
       $data = array(
                    'received_amount' => $total_amount,
                    );
       
       $this->db->where('id_salepayment', $sale_id)
                        ->where('inv_no', $inv_no)
                        ->update('sale_payment', $data);
                
       if($this->db->affected_rows()>0){
           $this->db->where('id_payment_reconciliation', $reconsilation)
                    ->where('idsale_payment', $sale_id)
                    ->where('inv_no', $inv_no)
                    ->delete('payment_reconciliation');
       }
    ...
    public function get_transaction_update_count($reconciliation,$idsale,$inv_no){
                  $data=$this->db->select('idpaymentreconciliation')
                        ->where('idpaymentreconciliation', $reconciliation)
                        ->where('idsale', $idsale)
                        ->where('inv_no', $inv_no)
                        ->get('credit_edit_history')->result();

                  $cnt_credit=count($data);
                  return $cnt_credit;
    }
    public function get_payment_modes_for_credit_received_edit_report() {
        return  $this->db->select('pm.payment_mode, pm.id_paymentmode, payment_head.payment_head')
                        ...
                        ->where('pm.id_paymentmode <> 1')
                        ->get('payment_mode pm')->result();
    } 
    public function ajax_get_cheque_reconciled_edit($idbranch, $idbranches, $datefrom, $dateto, $idmode) {
        if($idbranch == ''){
            $branch_arr = explode(',',$idbranches);
        }else{
            $branch_arr[] = $idbranch;
    ...
    public function payment_reconciliation_return() {
        //print_r($_POST);die;
        $paymet_recons_id = $_POST['payment_id'];
        $retunr_cnt = $_POST['return_count'];
        $idrole = $_POST['idrole'];
        $id_salepayment = $_POST['id_salepayment'];
        $id_paymenthead = $_POST['id_paymenthead'];
        $data_cnt = $this->db->select('s.return_count')                        
                    ->where('s.id_payment_reconciliation', $paymet_recons_id)
                    ->get('payment_reconciliation s')->row(); 
       
        $retunrcnt_increment = $data_cnt->return_count + 1;
        if($id_paymenthead !=2){
           
        $str1 = "UPDATE `payment_reconciliation` SET payment_receive=DEFAULT(payment_receive),transfer_date=DEFAULT(transfer_date),received_entry_time=DEFAULT(received_entry_time),received_amount=DEFAULT(received_amount),pending_amt=DEFAULT(pending_amt),idbank=DEFAULT(idbank),commission_amt=DEFAULT(commission_amt),commission_per=DEFAULT(commission_per),utr_no=DEFAULT(utr_no),received_by=DEFAULT(received_by),return_count=$retunrcnt_increment WHERE `id_payment_reconciliation`=$paymet_recons_id AND `idsale_payment`=$id_salepayment";
        $this->db->query($str1);    
        }else{
            
            $str1 = "UPDATE `payment_reconciliation` SET payment_receive=DEFAULT(payment_receive),transfer_date=DEFAULT(transfer_date),received_entry_time=DEFAULT(received_entry_time),received_amount=DEFAULT(received_amount),pending_amt=DEFAULT(pending_amt),idbank=DEFAULT(idbank),utr_no=DEFAULT(utr_no),received_by=DEFAULT(received_by),bank_reconciliation=DEFAULT(bank_reconciliation),return_count=$retunrcnt_increment WHERE `id_payment_reconciliation`=$paymet_recons_id AND `idsale_payment`=$id_salepayment";
            $this->db->query($str1);
            
            if($this->db->affected_rows()>0){
            $str2 = "UPDATE `sale_payment` SET payment_receive=DEFAULT(payment_receive),received_entry_time=DEFAULT(received_entry_time),received_amount=DEFAULT(received_amount),received_by=DEFAULT(received_by),short_receive=DEFAULT(short_receive),bank_reconciliation=DEFAULT(bank_reconciliation) WHERE `id_salepayment`=$id_salepayment";    
            $this->db->query($str2);
            }
        }
    ...
    public function ajax_bfl_data_form() {
        ...
        $data = $this->db->where('s.idsale', $id_sale)
                         ->get('bfl_file_customer s')->row();
         return $data;
    }
    public function ajax_get_credit_received_report_byfilter_edit($idpaymentmode, $idbranch, $datefrom, $dateto, $modes, $allbranches){
...
//        die(print_r($_POST));
        return $this->db->select('sale.final_total,sale.customer_fname,sale.date as invoice_date,sale.customer_lname,sale.customer_contact,payment_reconciliation.*,branch.branch_name,payment_mode.payment_mode,sale_payment.received_amount as payment_rec,sale_payment.amount as sale_amnt,branch.branch_code')                        
                        ->where('payment_reconciliation.payment_receive =', 0)
                        ->where('payment_reconciliation.date >=', $datefrom)
                        ->where('payment_reconciliation.date <=', $dateto)
                        ->where_in('payment_reconciliation.idpayment_mode', $modes_arr)
                        ->where_in('payment_reconciliation.idbranch', $branches)
