...
class Api_Model extends CI_Model {
    ...
    public function get_customer_bycontact($contact,$idbranch) {
       
        $q['customer'] = $this->db->where('customer_contact', $contact)->get('customer')->result();
        
        $modes = $this->db->where('billing_modes.active', 1)->get('billing_modes')->result();
        $str="Select ";
        foreach ($modes as $mode){
            $str.=" ".$mode->billing_mode_column_name.",";    
        }
        $str=rtrim($str,',');
        $str.=" from branch where id_branch = $idbranch";   
        
        $q['billing_modes'] = $this->db->query($str)->result();  
        return $q;
...
//        }
    ...
    public function get_get_billing_modes_bybranchid($idbranch) {     
        $modes = $this->db->where('billing_modes.active', 1)->get('billing_modes')->result();
        $str="Select ";
        foreach ($modes as $mode){
            $str.=" ".$mode->billing_mode_column_name.",";    
...
//		die($this->db->last_query());				
    ...
    public function get_lmtd_promotor_sale_report_byidbranch_idpromoter($month,$lastmonth, $idpcat,$idbranch,$idpromoter){
      ...
      public function get_promotor_target_ach_byidbranch_idpromoter($monthyear, $idpcat, $idbranch, $idpromoter){
            ...
            $this->db->where_in('b.id_branch', $branches);
            $this->db->where('b.is_warehouse',0);
            $this->db->join("(select sum(p.volume) as pvolume, sum(p.value) as pvalue,sum(p.asp) as pasp,p.revenue as prevenue, p.idbranch,p.idpromotor from promotor_target_setup p WHERE p.idpromotor='$idpromoter' and p.idbranch='$idbranch' and  p.month_year = '$month_year' and p.idproductcategory in($pcatss) GROUP BY p.idbranch, p.idpromotor ) prom", 'prom.idbranch = b.id_branch and prom.idpromotor = users.id_users', 'left');
            $this->db->join("(select sum(s.qty) as sale_qty, sum(s.total_amount) as total,sum(s.landing) as landing,s.idbranch,sale.idsalesperson from sale_product s,sale WHERE sale.idsalesperson='$idpromoter' and s.idbranch='$idbranch' and  s.idsale = sale.id_sale and s.date between '$from' and '$to' and s.idproductcategory in($pcatss) GROUP BY s.idbranch, sale.idsalesperson ) sp", 'sp.idbranch = b.id_branch and sp.idsalesperson = users.id_users', 'left');
        ...
        return $this->db->select('sale_token.*,  sale_token.entry_time as invoice_date,users.user_name,print_head.*,customer.*,branch.*')->where('id_sale_token',$idsaletoken)
    ...
    public function get_saletoken_product_byid($idsaletoken) {
        return $this->db->where('idsaletoken',$idsaletoken)->get('sale_token_product')->result();
    }
    public function get_saletoken_product_byid_group($idsaletoken) {
        return $this->db->select('sale_token_product.*,sum(qty) as qty')->where('idsaletoken',$idsaletoken)->group_by('idvariant')->get('sale_token_product')->result();
    }
    public function get_saletoken_payment_byid($idsaletoken) {
        return $this->db->where('idsaletoken',$idsaletoken)
                        ->where('sale_token_payment.idpayment_mode = payment_mode.id_paymentmode')->from('payment_mode')
                        ->where('sale_token_payment.idpayment_head = payment_head.id_paymenthead')->from('payment_head')
                        ->get('sale_token_payment')->result();        
    }
    public function get_sale_reconciliation_byinv($id){
        return $this->db->where('inv_no', $id)
     ...
     function short_url($url) {    
         ...
         $adata = array();
        $adata['key'] = "AIzaSyCsV4OaLPu_okJ0qZN8e7uh49oGc8T55go";
        $fields_string = "";
        if ($adata) {
        ...
        $a=rtrim($fields_string, '&');
        $data = array();
        $data['dynamicLinkInfo']['domainUriPrefix'] ="https://ssmobile.page.link";
        $data['dynamicLinkInfo']['link'] =$url;
        $e=json_encode($data);          
        $newurl = "https://firebasedynamiclinks.googleapis.com/v1/shortLinks?".$a;
//        die($newurl);
        $result = $this->rest->request($newurl, "POST", json_encode($data));        
    ...
    public function ajax_stock_data_byimei_branch($imei, $branch,$godown){
        return $this->db->where('imei_no', $imei)->where('idbranch', $branch)
                        ->where('stock.idgodown',$godown)
                        ->where('stock.idcategory = category.id_category')->from('category')
                        ->where('stock.idvariant = model_variants.id_variant')->from('model_variants')
                        ->get('stock')->result();
    }
