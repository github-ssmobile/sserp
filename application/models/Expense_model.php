...
//    }
    ...
    public function get_expense_data_bymonthyear($idbranch, $monthyear){
     ...
     public function get_all_expense_summary_report($idbranch, $from, $idwallet){
//        die(print_r($_POST));
        if($idbranch == 0){
            $id = $_SESSION['id_users'];
            if($this->session->userdata('level') == 1){  //admin all branch
         ...
         $strbr = implode(',', $branchid);
        
        
        $this->db->select('b.id_branch, b.branch_name, et.id_wallet_type, et.wallet_type, petti.total_cash,petti.month_year as month,ex.month_year,ex.exp_amt');
        $this->db->where_in('id_branch', $branchid);         
        if($idwallet != 0){
            $this->db->where_in('id_wallet_type', $idwallet);
        }
        $this->db->join("(select sum(p.amount) as total_cash,p.month_year, p.idbranch,p.idwallet_type from petti_cash p WHERE p.month_year = '$from' and p.idbranch in($strbr) GROUP BY p.idbranch, p.month_year,p.idwallet_type) petti", 'petti.idbranch = b.id_branch and petti.idwallet_type = et.id_wallet_type', 'left');
        $this->db->join("(select sum(e.approve_expense_amount) as exp_amt,e.month_year ,e.idbranch,e.id_wallet from expense e WHERE e.month_year = '$from' and e.idbranch in($strbr) GROUP BY e.idbranch, e.month_year,e.id_wallet) ex", 'ex.idbranch = b.id_branch and ex.id_wallet = et.id_wallet_type', 'left');
        $this->db->order_by('b.id_branch,et.id_wallet_type');
        $this->db->from('expense_wallet_type et');
