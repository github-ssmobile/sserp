...
//      $this->db->where('b.id_branch!=7');  
        if($idbranch){
            $this->db->where('b.id_branch', $idbranch);    
        } 
        $this->db->where('b.active', 1);         
        // enable all branches to both warehouses
        if ($idwarehouse == 7) {  }else{ // || $idwarehouse == 18
...
//        die(print_r($this->db->last_query()));
    ...
    public function delete_branch_allocation_by_allocationid($idstock_allocation){               
        $qy="delete sad from stock_allocation_data sad,stock_allocation sa  where sa.id_stock_allocation=$idstock_allocation and sad.idstock_allocation=$idstock_allocation  and sa.status < 3";                                   
        return $this->db->query($qy);            
    } 
...
//        die(print_r($this->db->last_query()));
    ...
    public function get_warehouse_stock_data($variantid,$modelid,$idproductcategory,$idbrand,$idgodown,$warehouse,$allocation_type,$godown_to) {                             
        $this->db->select('mv.*,mv.full_name,mv.idmodel,`b`.`id_branch`, `b`.`branch_name`, stk_ho.ho_stock_qty,sallo.allocated_qty,salloo.o_allocated_qty,callo.c_allocated_qty');
        $this->db->from('model_variants mv');
        if($variantid){
            $this->db->join('(select sum(st.qty) as ho_stock_qty,st.idbranch,st.idvariant from stock st WHERE  st.idgodown in('.$idgodown.','.$godown_to.') and st.idbranch='.$warehouse.' and st.idvariant='.$variantid.' GROUP BY st.idvariant) stk_ho','`stk_ho`.`idvariant` = `mv`.`id_variant`','left');                                 
            $this->db->join('(select sum(sad.qty) as allocated_qty,sad.idvariant from stock_allocation_data sad inner join stock_allocation sa on sad.idstock_allocation=sa.id_stock_allocation WHERE (sa.status < 3 )  and sa.idwarehouse='.$warehouse.'  and sad.idgodown in('.$idgodown.','.$godown_to.') and  sad.idvariant='.$variantid.'  GROUP BY sad.idvariant) sallo','`sallo`.`idvariant` = `mv`.`id_variant`','left');                 
            $this->db->join('(select sum(sad.qty) as o_allocated_qty,sad.idvariant from stock_allocation_data sad inner join stock_allocation sa on sad.idstock_allocation=sa.id_stock_allocation WHERE (sa.status < 3 )  and sa.idwarehouse='.$warehouse.'  and sad.idgodown in(6) and  sad.idvariant='.$variantid.'  GROUP BY sad.idvariant) salloo','`salloo`.`idvariant` = `mv`.`id_variant`','left');                 
            $this->db->join('(select sum(sad.qty) as c_allocated_qty,sad.idvariant from stock_allocation_data sad inner join stock_allocation sa on sad.idstock_allocation=sa.id_stock_allocation WHERE sa.status=0  and sa.idwarehouse='.$warehouse.'  and sa.allocation_type='.$allocation_type.' and sad.idgodown in('.$idgodown.') and  sad.idvariant='.$variantid.'  GROUP BY sad.idvariant) callo','`callo`.`idvariant` = `mv`.`id_variant`','left');                     
            $this->db->where('mv.id_variant',$variantid);    
        }else{
            $this->db->join('(select sum(st.qty) as ho_stock_qty,st.idbranch,st.idvariant from stock st WHERE  st.idgodown in('.$idgodown.','.$godown_to.') and st.idbranch='.$warehouse.' and st.idbrand='.$idbrand.' and  st.idproductcategory='.$idproductcategory.' GROUP BY st.idvariant) stk_ho','`stk_ho`.`idvariant` = `mv`.`id_variant`','left');                                 
            $this->db->join('(select sum(sad.qty) as allocated_qty,sad.idvariant from stock_allocation_data sad inner join stock_allocation sa on sad.idstock_allocation=sa.id_stock_allocation WHERE (sa.status < 3 ) and sa.idwarehouse='.$warehouse.' and sad.idgodown in('.$idgodown.','.$godown_to.') and  sad.idbrand='.$idbrand.' and  sad.idproductcategory='.$idproductcategory.'  GROUP BY sad.idvariant) sallo','`sallo`.`idvariant` = `mv`.`id_variant`','left');                 
            $this->db->join('(select sum(sad.qty) as o_allocated_qty,sad.idvariant from stock_allocation_data sad inner join stock_allocation sa on sad.idstock_allocation=sa.id_stock_allocation WHERE (sa.status < 3 ) and sa.idwarehouse='.$warehouse.' and sad.idgodown in(6) and  sad.idbrand='.$idbrand.' and  sad.idproductcategory='.$idproductcategory.'  GROUP BY sad.idvariant) salloo','`salloo`.`idvariant` = `mv`.`id_variant`','left');                 
            $this->db->join('(select sum(sad.qty) as c_allocated_qty,sad.idvariant from stock_allocation_data sad inner join stock_allocation sa on sad.idstock_allocation=sa.id_stock_allocation WHERE sa.status=0  and sa.idwarehouse='.$warehouse.'  and sa.allocation_type='.$allocation_type.' and sad.idgodown in('.$idgodown.') and  sad.idbrand='.$idbrand.' and  sad.idproductcategory='.$idproductcategory.'  GROUP BY sad.idvariant) callo','`callo`.`idvariant` = `mv`.`id_variant`','left');                     
            $this->db->where('mv.idproductcategory', $idproductcategory);
        }        
...
//        die(print_r($this->db->last_query()));
    ...
    public function get_same_variants_for_allocation($variantid,$warehouse,$idproductcategory,$idbrand,$idgodown,$allocation_type,$modelid,$type,$godown_to) { 
        $qy="";
        if($type==0){
            $qy="select sum(stk.qty) as ho_stock_qty,salloo.o_allocated_qty,sallo.allocated_qty,callo.c_allocated_qty,mv.* from model_variants mv left join stock stk on stk.idvariant=mv.id_variant and stk.idgodown in($idgodown,$godown_to) and idbranch=$warehouse left join "
                ...
                . "(select sum(sad.qty) as o_allocated_qty,sad.idvariant from stock_allocation_data sad inner join stock_allocation sa on sad.idstock_allocation=sa.id_stock_allocation WHERE (sa.status < 3) and sa.idwarehouse=$warehouse and sad.idgodown in(6) and  sad.idbrand=$idbrand and  sad.idproductcategory=$idproductcategory  GROUP BY sad.idvariant) salloo on `salloo`.`idvariant` = `mv`.`id_variant` left join "
            ...
            $qy="select sum(stk.qty) as ho_stock_qty,sallo.allocated_qty,salloo.o_allocated_qty,callo.c_allocated_qty,mv.* from model_variants mv left join stock stk on stk.idvariant=mv.id_variant and stk.idgodown in($idgodown,$godown_to) and idbranch=$warehouse left join "
                . "(select sum(sad.qty) as allocated_qty,sad.idvariant from stock_allocation_data sad inner join stock_allocation sa on sad.idstock_allocation=sa.id_stock_allocation WHERE (sa.status < 3)  and sa.idwarehouse=$warehouse  and sad.idgodown in($idgodown,$godown_to) and  sad.idbrand=$idbrand and  sad.idproductcategory=$idproductcategory  GROUP BY sad.idvariant) sallo on `sallo`.`idvariant` = `mv`.`id_variant` left join "
                . "(select sum(sad.qty) as o_allocated_qty,sad.idvariant from stock_allocation_data sad inner join stock_allocation sa on sad.idstock_allocation=sa.id_stock_allocation WHERE (sa.status < 3)  and sa.idwarehouse=$warehouse  and sad.idgodown in(6) and  sad.idbrand=$idbrand and  sad.idproductcategory=$idproductcategory  GROUP BY sad.idvariant) salloo on `salloo`.`idvariant` = `mv`.`id_variant` left join "    
                . "(select sum(sad.qty) as c_allocated_qty,sad.idvariant from stock_allocation_data sad inner join stock_allocation sa on sad.idstock_allocation=sa.id_stock_allocation WHERE sa.status=0  and sa.idwarehouse=$warehouse  and sa.allocation_type=$allocation_type and sad.idgodown in($idgodown) and  sad.idbrand=$idbrand and  sad.idproductcategory=$idproductcategory  GROUP BY sad.idvariant) callo on `callo`.`idvariant` = `mv`.`id_variant` "
...
//        die(print_r($this->db->last_query()));
    ...
    public function get_branch_allocation_stock_data_newonline($branch,$warehouse,$allocation_type,$idvariant,$idgodown,$idgodown_to) {  // NEW AND ONLINE
        $this->db->select('sum(sd.qty) as allocated_qty,stk_ho.ho_stock_qty');
        $this->db->from('stock_allocation_data sd');                
        $this->db->from('stock_allocation sa');        
        $this->db->where('sd.idstock_allocation=sa.id_stock_allocation'); 
        $this->db->where('sd.idvariant',$idvariant);   
        $this->db->join('(select sum(st.qty) as ho_stock_qty,st.idbranch,st.idvariant,st.idgodown from stock st WHERE st.idbranch='.$warehouse.' and st.idgodown=1 and st.idvariant='.$idvariant.' GROUP BY st.idvariant) stk_ho','`stk_ho`.`idvariant` = `sd`.`idvariant`','left');                                                                  
        $this->db->where('sa.status < 3');       
        $this->db->where('sd.idgodown in ('.$idgodown.','.$idgodown_to.')');         
        $this->db->where('sa.idwarehouse',$warehouse);         
        $this->db->group_by('sd.idvariant');
        $query = $this->db->get(); 
        return $query->result(); 
//        die(print_r($this->db->last_query()));
    }
    
    public function get_branch_allocation_stock_data_byvariantid_new_online($branch,$days,$warehouse,$allocation_type,$idvariant,$idgodown,$idgodown_to) {  // NEW AND ONLINE
        $to=date('Y-m-d');
        $day='-'.$days.' days';                 
        $from=date('Y-m-d', strtotime("$day", strtotime($to)));  
        $this->db->select('u.user_name,u.id_users,sd.id_stock_allocation_data,snn.norm_qty,stk_ho.ho_stock_qty,sallo.allocated_qty,callo.c_allocated_qty,ballo.callocated_qty,sp.sale_qty,stkm.intra_stock_qty,bstk.stock_qty,g.*,mv.*');
        $this->db->from('stock_allocation_data sd');        
        $this->db->join('(select sum(st.qty) as ho_stock_qty,st.idbranch,st.idvariant,st.idgodown from stock st WHERE  st.idvariant='.$idvariant.' and st.idgodown in ('.$idgodown.','.$idgodown_to.') and st.idbranch='.$warehouse.' GROUP BY st.idvariant) stk_ho','`stk_ho`.`idvariant` = `sd`.`idvariant`','left');                                 
        $this->db->join('(select sum(sad.qty) as allocated_qty,sad.idvariant,sad.idgodown from stock_allocation_data sad inner join stock_allocation sa on sad.idstock_allocation=sa.id_stock_allocation WHERE sad.idvariant='.$idvariant.' and sad.idgodown in ('.$idgodown.','.$idgodown_to.') and (sa.status < 3) and sa.idwarehouse='.$warehouse.'  GROUP BY sad.idvariant) sallo','`sallo`.`idvariant` = `sd`.`idvariant`','left');
        $this->db->join('(select sum(sad.qty) as c_allocated_qty,sad.idvariant,sad.idgodown from stock_allocation_data sad inner join stock_allocation sa on sad.idstock_allocation=sa.id_stock_allocation WHERE sad.idgodown='.$idgodown.' and  sa.status=0  and sa.idwarehouse='.$warehouse.'  and sa.allocation_type='.$allocation_type.' GROUP BY sad.idvariant,sad.idgodown) callo','`callo`.`idvariant` = `sd`.`idvariant` and callo.idgodown=sd.idgodown','left');                     
        $this->db->join('(select sum(st.qty) as stock_qty,st.idbranch,st.idvariant,st.idgodown from stock st WHERE  st.idbranch='.$branch.' GROUP BY st.idvariant,st.idgodown) bstk','`bstk`.`idgodown` = `sd`.`idgodown` and `bstk`.`idvariant` = `sd`.`idvariant`','left');                 
        $this->db->join('(select sum(stm.qty) as intra_stock_qty,stm.temp_idbranch,stm.idvariant,stm.idgodown from stock stm WHERE  stm.temp_idbranch='.$branch.' GROUP BY stm.idvariant,stm.idgodown) stkm',' `stkm`.`idgodown` = `sd`.`idgodown` and `stkm`.`idvariant` = `sd`.`idvariant`','left');         
        $this->db->join('(select sn.quantity as norm_qty,sn.idbranch,sn.idvariant from stock_norms sn WHERE sn.idbranch='.$branch.' GROUP BY sn.idvariant,sn.idbranch) snn','`snn`.`idbranch` = `sd`.`idbranch` and `snn`.`idvariant` = `sd`.`idvariant`','left');        
        $this->db->join("(select sum(s.qty) as sale_qty,s.idbranch,s.idvariant,s.idgodown from sale_product s WHERE s.date between '$from' and '$to' and s.idbranch=$branch GROUP BY s.idvariant,s.idgodown) sp", '`sp`.`idgodown` = `sd`.`idgodown` and `sp`.`idvariant` = `sd`.`idvariant`', 'left');                
        $this->db->join('(select sad.qty as callocated_qty,sad.idbranch,sad.idvariant,sad.idgodown from stock_allocation_data sad inner join stock_allocation sa on sad.idstock_allocation=sa.id_stock_allocation WHERE  sad.idgodown='.$idgodown.' and sa.status=0  and sa.idwarehouse='.$warehouse.'   and sa.allocation_type='.$allocation_type.' and sa.idbranch='.$branch.') ballo','`ballo`.`idvariant` = `sd`.`idvariant`','left');                 
        $this->db->join('model_variants mv','mv.id_variant=sd.idvariant');                
        $this->db->where('sa.status',0)->where('sa.allocation_type', $allocation_type)->where('sa.idbranch', $branch)->from('stock_allocation sa');
        $this->db->where('g.id_godown',$idgodown)->from('godown g');
        $this->db->where('sd.idstock_allocation=sa.id_stock_allocation'); 
        $this->db->where('sa.idwarehouse',$warehouse); 
...
//         die(print_r($this->db->last_query()));
	 ...
	 public function get_active_branchs_forallocation($idbrand){
         $this->db->select('*')->where('b.active', 1);        //->where('b.id_branch!=7')
         $this->db->join('branch_category bc','b.idbranchcategory = bc.id_branch_category','left');                
         $this->db->join('zone z','b.idzone = z.id_zone','left');   
