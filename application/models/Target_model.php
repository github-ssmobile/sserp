...
//            $c_to = date('Y-m-t'); 
        ...
        public function get_promotor_target_ach_byidbranch($monthyear, $idpcat, $allpcats, $idbranch, $allbranches){
            ...
            $this->db->join("(select sum(s.qty) as sale_qty, sum(s.total_amount) as total,sum(s.landing) as landing,s.idbranch,sale.idsalesperson from sale_product s,sale WHERE s.idsale = sale.id_sale and s.date between '$from' and '$to' and s.idproductcategory in($pcatss) GROUP BY s.idbranch, sale.idsalesperson ) sp", 'sp.idbranch = b.id_branch and sp.idsalesperson = users.id_users', 'left');
            $this->db->join("(select sum(ss.qty) as smart_qty,sum(total_amount) as smart_amount,ss.idbranch,sale.idsalesperson from sale_product ss,sale WHERE ss.idsale = sale.id_sale and ss.date between '$from' and '$to' and  ss.idcategory in(1,32) and ss.idproductcategory in($pcatss) GROUP BY ss.idbranch, sale.idsalesperson ) smartsp", 'smartsp.idbranch = b.id_branch and smartsp.idsalesperson = users.id_users', 'left');
        ...
        public function get_promotor_target_ach_byidbzone($monthyear, $idpcat, $allpcats, $idzone,$allzones){
            ...
            $this->db->join("(select sum(sss.qty) as rudram_qty,sss.idbranch,sale.idsalesperson from sale_product sss,sale WHERE sss.idsale = sale.id_sale and sss.date between '$from' and '$to' and sss.idbrand = 29  GROUP BY sss.idbranch, sale.idsalesperson ) rudram", 'rudram.idbranch = b.id_branch and rudram.idsalesperson = users.id_users', 'left');
            $this->db->join("(select count(spay.id_salepayment) as finance_qty,spay.idbranch,sale.idsalesperson from sale_payment spay,sale_product, sale WHERE spay.idsale = sale.id_sale and sale_product.idsale = spay.idsale   and spay.idpayment_head = 4 and spay.date between '$from' and '$to' and sale_product.idcategory in(1,32) and sale_product.idproductcategory in($pcatss) GROUP BY spay.idbranch, sale.idsalesperson ) finance", 'finance.idbranch = b.id_branch and finance.idsalesperson = users.id_users', 'left');
            $this->db->join("(select sum(s.qty) as pro_qty,s.idbranch,sale.idsalesperson from sale_product s,sale WHERE s.idsale = sale.id_sale and s.date between '$from' and '$to' and s.idproductcategory = 8 GROUP BY s.idbranch, sale.idsalesperson ) pro_plan", 'pro_plan.idbranch = b.id_branch and pro_plan.idsalesperson = users.id_users', 'left');
            $this->db->where('users.iduserrole', 17);
            $this->db->where('users.active', 1);
            $this->db->where('b.is_warehouse',0);
            $this->db->order_by('zone.id_zone,b.id_branch');
...
//        die($this->db->last_query());
    ...
    //********* Allocator Target*/////
     public function get_allocators_data(){
        return $this->db->where('active',1)->where('iduserrole', 13)->get('users')->result();
    }
    public function get_allocator_has_branch_byid($iduser){
        return $this->db->where('iduser', $iduser)
                        ->where('branch.idzone = zone.id_zone')
                        ->where('idbranch = branch.id_branch')
                        ->order_by('zone.id_zone, branch.id_branch')
                        ->from('zone')->from('branch')
                        ->get('user_has_branch')->result();
    }
    public function save_allocatore_target_data($data){
        return $this->db->insert_batch('allocator_target', $data);
    }
    public function update_allocatior_target_data($data, $idallocator){
        return $this->db->where('id_allocator_target', $idallocator)->update('allocator_target', $data);
    }

    public function get_allocation_target_data($iduser, $monthyear){
        $this->db->select('z.id_zone,z.zone_name,b.id_branch,b.branch_name,pset.id_allocator_target,pset.qty');
        $this->db->where('ub.iduser', $iduser);
        $this->db->join("(select atr.id_allocator_target, atr.idbranch, atr.qty, atr.iduser from allocator_target atr WHERE atr.month_year = '$monthyear' and atr.iduser = $iduser) pset", 'pset.idbranch = b.id_branch and pset.iduser = ub.iduser', 'left');
        $this->db->where('ub.idbranch = b.id_branch');
        $this->db->where('b.idzone = z.id_zone');
        $this->db->order_by('z.id_zone, b.id_branch');
        $this->db->from('branch b');
        $this->db->from('zone z');
        $this->db->from('user_has_branch ub');
        $query = $this->db->get();  
        return $query->result();
//         die($this->db->last_query());
    }
    public function get_allocation_target_vs_ach_data($iduser, $monthyear){
        $from = $monthyear.'-01';
        $to = date('Y-m-t', strtotime($from));
        
        $this->db->select('u.user_name,bc.branch_category_name,z.id_zone,z.zone_name,b.id_branch,b.branch_name,pset.qty,allc.allocated_qty');
        $this->db->where('u.id_users', $iduser);
        $this->db->join("(select atr.id_allocator_target, atr.idbranch, atr.qty from allocator_target atr WHERE atr.month_year = '$monthyear' and atr.iduser = $iduser) pset", 'pset.idbranch = b.id_branch', 'left');
        $this->db->join("(select sum(sd.qty) as allocated_qty, sd.idbranch, sd.created_by from stock_allocation_data sd WHERE sd.date between '$from' and '$to' and sd.created_by = $iduser group by sd.idbranch, sd.created_by) allc", 'allc.idbranch = b.id_branch and allc.created_by = u.id_users', 'left');
        $this->db->where('ub.idbranch = b.id_branch');
        $this->db->where('b.idzone = z.id_zone');
        $this->db->where('u.id_users = ub.iduser');
        $this->db->where('b.idbranchcategory = bc.id_branch_category');
        $this->db->order_by('z.id_zone, b.id_branch');
        $this->db->from('branch b');
        $this->db->from('branch_category bc');
        $this->db->from('zone z');
...
//         die($this->db->last_query());
    ...
    public function check_allocation_target_data($iduser, $monthyear){
        return $this->db->where('iduser', $iduser)
                        ->where('month_year',$monthyear)
                        ->get('allocator_target')->result();
    }
